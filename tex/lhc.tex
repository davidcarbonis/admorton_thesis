...
The vast majority of events are uninteresting from a physics perspective, with the cross sections of interesting processes being at least $\approx 10^{7}$ smaller than the total proton-proton cross section of $110.6 \pm3.4\mb$~\cite{Antchev:2017dia}. %the CMS trigger system rejects these background events.
The challenge therefore is to meet the 
constraint of the design readout and storage capacity of 100\Hz whilst 
Possible signatures of new physics

st having acceptance thresholds which are sufficiently low enough to allow ... 

The CMS trigger system does this selecting these ``interesting'' events for storage for later analysis, and is comprised of two stages: The Level-1 (L1) Trigger and the High Level Trigger (HLT).

\paragraph{L-1 Trigger}\label{paragraph:L1}
The Level-1 Trigger consists of custom-designed programmable hardware (FPGA technology where possible). 
...
\paragraph{High Level Trigger}\label{paragraph:HLT}
