...
With the solenoid's coil acting as an additional absorber, the HO increases the effective absorber thickness to at least 11.8 interaction lengths.
...
Outside the solenoid, an iron return yoke guides and contains the return magnetic field as illustrated in figure~\ref{fig:magneticField}.
...
Given that muons are Minimum Ionising Particles (MIPs), they pass through the inner detectors and the solenoid with minimal interaction.
...
The DTs are marked in yellow, the CSCs in green and the RPCs in blue.
Prior to the start of Run-II of the LHC in 2015, during LS1 the outer fourth CSC and fourth RPC disks were installed~\cite{CMS-DP-2016-046}.}
...
Each tube is a $4.2\cm \times 1.3\cm$ cell that contains positively charged wire surrounded by a mixture of argon (85\%) and carbon dioxide (15\%).
As a muon passes through the chamber it ionises the gas within, with the resultant free electron drifting towards the 

Each cylindrical layer of DT chambers is organised into four stations, where the chambers 

 and for each station, the chambers are subdivided into three ``superlayers'' (/SLs).
...
Cathode Strip Chambers (CSCs) are employed in the endcaps covering the region $0.9 < \eta < 2.4$, where the magnetic field is non-uniform and there is a higher rate of muons than in the barrel.
The CSCs are organised into four stations of concentric disks.

Initially only the innermost ring of the outermost (fourth) disk was installed.
An outer ring for the outermost disk was installed during LS1 during 2013-2015~\cite{Battilana:2017mrm}.

Each CSC is composed of six gas gaps, with six planes of anode wires running almost perpendicular to six planes of cathode strips.
This provides six position measurements per chamber with a resolution in the $r-\phi$ plane of 75\mum for the first station and 150\mum for the other station~\cite{CMS:1997iti}.

Resistive Plate Chambers (RPCs) provide coverage in the range $|\eta| < 1.8$. 
Originally the RPCs only extended to $|\eta| < 1.6$, instead of the planned $|\eta| < 2.1$, for financial reasons. 
During LS1, a fourth endcap disk instrumented with RPCs was installed that extended the coverage~\cite{Battilana:2017mrm}.

In contrast to the DTs and CSCs, RPCs have a coarser position resolution of about 1\cm but 
Although the RPCs have a coarser position resolution of $\approx 1\cm$) than the DTs and CPCs, they have fast response times and excellent time resolution ($\approx 2\ns$).

As such, RPCs provide additional but complimentary input to trigger decisions.
Each RPCs is formed of two parallel resistive plates, separated by a few millimetres, with a large electric field applied across the gas filled gap.
...
When the information from the muon and tracker systems are combined, as described in Chapter~\ref{chapter:data-mc}, 

a momentum resolution of $1.3\%$ to $2.0\%$ and up to $6\%$ in the barrels and endcaps, respectively, and a charge misidentification rate of less than $0.1\%$ for muons with transverse momenta less than $100 \GeV$~\cite{Chatrchyan:2012xi,Chatrchyan:2013sba}.
