\textit{•}\chapter{The LHC accelerator and the CMS experiment}\label{chapter:lhc-cms}
...
\editComment{START: New TMUX text - required for TMTT chapter}
For the calorimeter trigger, this requirement has been met by a flexible design based on a large FPGAs on a small number of general-purpose boards which are able to perform more sophisticated identification and reconstruction algorithms over the entire detector without regional boundaries.
This \emph{time-multiplexed} trigger design differs from the previous traditional regional reconstruction data reduction architecture, which forwards the best candidates at each regional level to the next stage covering a larger region of the detector, by buffering and transmitting all the data for every ``n''th event to one of ``n'' identical processors.

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.97\textwidth]{figs/cms/TMT.}
\caption{~\cite{tmttelba}.}
\label{fig:TMT}
\end{center}
\end{figure}

The success of this approach has motivated a similar time-multiplexed approach for a potential track finding system for the Phase-II Outer Tracker, as discussed in~\ref{sec:An FPGA Based Track Finding Architecture and Processor}.
\editComment{END: New TMUX text - required for TMTT chapter}
