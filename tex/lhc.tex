...
%%% Rewrite
As the trigger decision cannot be made before the subsequent BX, the L1 Trigger uses a pipelined approach

 which uses coarse information from the calorimeters and muon chambers to 
...
The strict time limitations on how long it takes for the data can be collected and read out, precludes both the reading out of events in full and of the use of iterative reconstruction algorithms.
 ...
 that provides a total system latency of about 3.8\mus.
While a decision is being made, the detector data is buffered (the Tracker and ES buffers being the limiting factor on the system latency) prior to either being read out or discarded.
...
As it was planned for the the LHC to increase both its instantaneous luminosity and centre-of-mass following LS1, the Phase-I Trigger Upgrade~\cite{Tapper:2013yva} was undertaken to ensure that the 100\kHz L1 trigger limit would be maintained.
...
Figure~\ref{fig:trigger} illustrates how data from the sub-detector systems passes through the various stages of the Phase-I Level-1 trigger architecture.
Initially the \emph{calorimeter trigger} searches for energy deposits 

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.97\textwidth]{figs/cms/TrigUpgradeBlockDiagram.pdf}
\caption{The data flow for the Phase-I Level-1 trigger architecture~\cite{Tapper:2013yva}.}
\label{fig:trigger}
\end{center}
\end{figure}
