...
\item One or two of the selected jets are considered to be b-tagged by the CSVv2 tagging algorithm at the medium WP and are within $|\eta| < 2.4$.
...
\ttbar events which decay leptonically form the second largest background process.
...
Table~\ref{tab:ttbarCR} shows that after these selection criteria and the required simulation corrections discussed in Section~\ref{sec:simCorrections} have been applied, the resulting control region has a 94.8\% purity and that half of the backgrounds are from tW single top production.
...
CMS:2016tlj	 TheATLAScollaboration:2016myw
For this analysis this is achieved using a $\chi^{2}$-like variable that was inspired by 

 that depends on both the reconstructed top quark mass and the W boson mass for when the distributions of data and MC are compared.
...
\editComment{Update values are added, add plots}
...
\item the transverse impact parameter, $d_{xy}$, of the electron must be less than 0.05\cm in the barrel and 0.10\cm in the endcap disks.
\end{itemize}
...
The  while maintaining a high selection efficiency for real jets by requiring all jets to have part of their energy deposited in both the ECAL and HCAL and be composed of more than one particle.
...
The follwowing criteria are additionally applied for jets for $\eta \leq 2.40$:
\begin{itemize}
...
The CSVv2 tagging algorithm described in Section~\ref{subsec:objReco-bJets} is used to tag jets as being initiated by a b-quark, with a working point (WP) cut applied to the b-tag discriminator.
If the value of a jet's discriminator exceeds that of the Medium WP and has $|\eta| < 2.40$, the jet is considered to be a b-jet.
Based on the \emph{Loose}, \emph{Medium} and \emph{Tight} WPs definitions~\cite{Sirunyan:2017ezt}, given in table~\ref{tab:bTagWPs} in Section~\ref{subsec:objReco-bJets}, the Medium WP was chosen.
...
In contrast to the other tZq searches~\cite{Aaboud:2017ylb,Sirunyan:2017nbr} where the W boson decays into a lepton and its associated antineutrino, the W boson in the dilepton final state decays hadronically, allowing for the top quark to be fully reconstructed.
