...
While the \ttbar in association with a vector boson processes have cross sections many orders of magnitude smaller than \ttbar and single top production, only \ttbarZ is expected to be a significant background.
Only \ttbarW events where two leptons are produced which are consistent with a Z boson decay will pass the event selection criteria and the Z boson mass cut window will reject the vast majority of events produced by 
\ttbarH (and tHq).
%%%EXPLAIN
The fully hadronic decay of the top-anti-top pair and leptonic Z boson decay for \ttbarZ results in an identical topology to 

While the dilepton final state of a single top produced in association with both a W and Z boson, tWZ, will be indistinguishable from the tZq final state.
As the tWZ cross section is an order of magnitude smaller than the tZq cross section, this will be one of the minor backgrounds.
