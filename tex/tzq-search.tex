...
While the \ttbar in association with a vector boson processes have cross sections many orders of magnitude smaller than \ttbar and single top production, only \ttbarZ where the Z boson decays leptonically is expected to be a significant background.
The \ttbarZ (Z$\rightarrow$ll) final state results in an identical topology which is particularly difficult to discriminate against as, like the signal process,  b-jet  
%%%EXPLAIN
%%% ttZ ->2l2nu is for Z->ll or nunu
%%% ttZ -> 2q  is for Z->qq
%%% 2b's + 4q
