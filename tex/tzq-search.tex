...
\iten \itembf{$\Delta \eta_{seed}$ and $\Delta \phi_{seed}$} - the distances in $\eta$ and $\phi$ between the ECAL supercluster and where the track has been extrapolated to from the primary vertex.
...
\item \textbf{$I^{rel}_{EA}$} the relative isolation of the electron with effective area pileup alleviation, which is described further in Chapter~\ref{subsubsec:relIso}.
...
The tight isolation cut applied to the resultant collection of tight muons is 95\% efficient, and rejects muons that have a relative isolation, with $\Delta\beta$ pileup corrections, greater than 0.15.
...
A relative isolation variable $I^{rel}$ is defined in order to:
\begin{itemize}
...
Two different methods of estimating the pileup contamination present are typically used.

In the analysis presented, electrons use the $\rho$ * effective area ($rho * A_{eff}$) technique.
...
Muons employ the $\Delta\beta$ pileup mitigation technique, where the average pileup energy inside the cone is estimated as half of that originating from charged hadrons.
