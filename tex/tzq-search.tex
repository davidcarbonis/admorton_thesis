...
While no additional event selection cuts are applied to the top quark candidate, the various quantities which could be constructed from it, such as its invariant mass or the difference in $\phi$ between it and the Z boson candidate, were used by the multivariate analysis discussed in Chapter~\ref{sec:mvas}
...
As the tZq dilepton final state is distinguished by the presence of exactly two leptons from the Z boson decay, two quarks from the W boson decay, one recoil quark
