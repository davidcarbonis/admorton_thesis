...
The main other to production backgrounds are expected to be 

Single top production in the tW-channel is expected to be 

The lack of a real Z boson decaying into two leptons for the rarer top production processes of \ttbarW, \ttbarH will
The fully hadronic decay of the top-anti-top pair in \ttbar though 
...
Both the leading and next-to-leading order diagrams for the dilepton final state ... 35pb >> tZq xc. ISR + FSR required to meet jet cuts. Check nJets for tW ...
t and s-channel require significant amounts of gluon splitting and mis-ID'ed leptons


tW - z mass cut and number of jets cuts will reduced bkg. Lots of 3 jet events, but most 4+jets events rejected by loose ID cuts
...
Check ttbarZ and ttbarW and ttbarH yields in ee/mumu
ttbarW -> lnu = 2.6/4.7
ttbarW -> 2q = 3.1/4.2
ttbarZ -> 2l2nu = 51.2/93.2
ttbarZ -> 2q = 3.6/7.7
ttH bb = 1.2/2.7
ttH nonbb = 2.6/5.5
...
While the dilepton final state of a single top produced in association with both a W and Z boson, tWZ, will be indistinguishable from the tZq final state, as

The even rarer processes of tHq and tWZ are 
The rare signal top processes of tHq and tWZ have cross sections
tHq - similar xc to tZq, Z mass cut against H - very small actual contributions
tWZ - x7 times smaller xc than tZq, real W + Z will be indistinguishable
...
The diboson processes involving the presence of a leptonically decaying Z decay and either a W or Z boson hadronically decaying with additional jets from gluon splitting are anticipated to be the predominant source of background from these processes...	

In light of the small production cross sections of the tribosonic backgrounds, even when their leading order final states are consistent with the tZq event selection criteria, the contributions from these processes are anticipated to be among the few minor backgrounds.
...
Both $\sigma_{\mathrm{t}}$ and $\sigma_{\mathrm{W}}$ were determined by taking the standard deviation of the Gaussian fitted to the top quark and W boson mass peaks respectively.
\editComment{recall a more precise/sophisticated method was suggested?}

Using the simulated samples, cuts on this $\chi^{2}$-like variable were chosen to define a blinded signal region and and a \emph{side band} region containing signal-like events.


In order to balance the need for the blinded signal region to contain the majority of the signal process while minimising background contamination and a \emph{side band} region which contains signal-like events and rejects any extreme outliers.
These cut values or 
