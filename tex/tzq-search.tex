...
Both the zero b-tagged jets and \MET and inverted W boson mass cut selection criteria produce highly enriched Z+jets control regions, xx\% and yy\% as given in Tables~\ref{tab:zPlusControlYieldsOld} and~\ref{tab:zPlusControlYieldsNew} respectively.
As the control region defined by the \MET and inverted W boson mass cut selection criteria was expected to more closely resemble the signal region's topology, it was decided that this control region would be used for the analysis.

\begin{table}[htbp]
\topcaption{The number of observed events in data and the number of expected events in simulation in the signal region fllowing the full event selection and simulation corrections for each of the separate channels and with both channels combined for the Z+jets control region requring zero b-tagged jets.
...
Z + jets (LO) : & 3293.634 & 6245.353 & 9538.987    \\
\hline
NPLs: & 37.787 & 30.889 & 68.676   \\
\hline
Data & 5284 & 9665 & 14949    \\
\hline
Total MC (LO Z+jets) & 5284.867 & 10067.752 & 15352.619    \\
Total MC (LO Z+jets) + NPLs & 5322.654 & 10098.641 & 15361.377    \\
\hline
\end{tabular}
\end{table}
...
Table~\ref{ttbarCR} shows that following the following these selection criteria and the application of all of the simulation corrections discussed in Chapter~\ref{sec:simCorrections}, that the resultant control region at 94\% purity is sufficiently enriched.

\begin{table}[htbp]
\topcaption {
The event yields after the application of following the full \ttbar control region event selection and simulation corrections for the \ttbar control region.
}
\label{tab:ttbarCR}
  \centering
% This right-aligns numbers in column, but centers them under column title.
