...
Before unblinding a $\chi^{2}$-like variable is used to define a side-band region outside the expected signal region in which data and simulation distributions can be compared and pseudo-data is used to evaluate the expected outcomes of the measurement, as discussed in Chapter~\cite{chapter:results}.
...
For $\sigma_{\mathrm{t}} = 8\GeV$ and $\sigma_{\mathrm{W}}) = 30\GeV$, the optimised values of $\chi^{2}_{SR}$ and $\chi^{2}_{SB}$ were determined to be 20 and 250, respectively.
