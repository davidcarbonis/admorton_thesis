...
In this analysis this is achieved by defining a \emph{side-band} region when comparing data and simulation 
 which does and only using pseudo-data
...
using a $\chi^{2}$-like variable, inspired by the one defined in~\cite{CMS:2016tlj,TheATLAScollaboration:2016myw}, that depends on both the reconstructed top quark mass and the W boson mass was constructed.
...
For the , the optimised values of $\chi^{2}_{SR}$ and $\chi^{2}_{SB}$ were determined to be 20 and 250  respectively.
