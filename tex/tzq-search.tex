...
From this high purity, large statistics lepton sample, the method 
...
These corrections are derived with high \pt ($> 20\GeVc$) muons from Z $ \rightarrow \mu\mu$ decays using a two step method.
The first step requires the mean inverse transverse momenta of muons from data and the reconstructed MC to be the same as the corresponding values from 
W
...
While the number of \PU interactions is simulated in MC, the number of primary vertices in simulation is weighted 
 data is dependent on the instantaneous luminosity 
 \PU interactions included in simulation
