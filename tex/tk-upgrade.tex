...
Figure~\ref{fig:kfHelixParametersResVsInvPt} shows the resolutions of the helix parameters as a function of track $\frac{1}{\pT}$ in simulation for both of the \MS coefficients considered and for when \MS was not accounted for at all in the \KF covariance matrix.

%%%
The negligible ...  the effects of \MS dominate at low \pT (high $\frac{1}{\pT}$)

As incorporating \MS increased the measurement error in the \rphi plane, impacting the  the negligible difference  


%% forecast of z0 depends on phi0, tanL depends only on tanL

Whilst incorporating \MS 
there were negligible differences in the $z_{0}$ and cot$\theta$ resolutions for  
Except No significant improvement in the resolutions of any of the helix parameters was observed ... 
...
%%
% phi0 = no difference at high pT but MS better at low pT
%% Greater weighting 
% z0 = slightly better at high pT? k(const) is best at low pT
% cot(theta) = k (layer) and k = 0 is roughly the same and k(const) is arguably better
% So both constants better at low pT for phi0, but 

In contrast, the resolutions of the other three helix parameters were not degraded by the inclusion of either of the \MS process noise terms.
Whilst there is little improvement in 
...
The development the \emph{TMTT} collaboration's proposed track finding system for the CMS Phase-II Outer Tracker included software studies into the performance of several track fitting algorithms and the robustness of the system when reducing the minimum track \pT threshold from 3\GeV to 2\GeV.

A software implementation of a linearised $\chi^{2}$ track fitting algorithm was developed and shown to be capable of efficiently finding tracks and fitting precise helix parameters to the track candidates found by the \HT.
Following the parallel development of both the linearised $\chi^{2}$ and the \KF track fitting algorithms however, 
