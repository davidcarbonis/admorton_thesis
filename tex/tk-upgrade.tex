...
\caption{The post-\HT (left) and post-\KF (right) tracking efficiency for $\pt > 2\GeVc$ for \ttbar events at <PU> of 200, with only the number of \qpt columns increased (red) and with the increased number of columns, HT cell merging and \KF state cuts optimisation(blue).
}
...
Figure~\ref{fig:htHelixParametersResVsInvPt} shows that the resolutions of the track parameters fitted by the \KF are comparable for $\pT < 3\GeV$ for both before and after the \HT and associated \KF optimisations for tracks originating from the primary interaction in \ttbar events at <PU> of 200.
...
\caption{
The \pt relative resolution, $\phi$ resolution, $z_{0}$ resolution and cot$\theta$ resolution measured both before the \HT and \KF optimisations (red) and afterwards (blue) for primary reconstructed tracks in simulated \ttbar events at <PU> of 200 for tracks with $\pt > 2\GeVc$.
\editComment{Replace \qpt resolution plot with relative \pt resolution plot once debugged root files are produced.}
\editComment{Make plots bigger!}
}
...
\begin{figure}[htb]
\centering
\includegraphics[width=0.49\textwidth]{figs/tk-upgrade/results-lowPtTracking/ptResVsInvPtTiltedGeometry_5000.pdf}
...
%\includegraphics[width=\textwidth]{figs/tk-upgrade/results-lowPtTracking/kfTrackingEffVsInvPtFlatGeometry_5000.pdf}
\caption{$\frac{\chi^{2}}{ndf}$ as a function of $\frac{1}{\pT}$ for for \ttbar events at <PU> of 200 after the full chain has been run, where the \KF has not been modified to take \MS into account (red), a constant coefficient for \MS is used (black) and a layer dependent coefficient for \MS is used (blue).
}
