...
This process involved developing approximated expressions of the track derivatives which reduced the number of variables they depended on, but still gave performance comparable to when floating point precision was used.
...
While the \qpt and $\phi_{0}$ resolutions achieved after four fitting iterations was comparable to those achieved by the \KF, the $\chi^{2}$ fitter's resolution was considerably worse for both the $z_{0}$ and $cot(\theta)$ resolutions in the endcaps.
...
With the particles involved having relativistic velocities (\ie $\beta c \cong 1$) and scattering in the \r-z plane ignored as the impact of multiple scattering is considerably smaller hit position resolution in \rz, the multiple scattering contribution in the \rphi plane can be expressed as:
...
Figure~\ref{fig:2GeVTiltEff} and Table~\ref{tab:trackFindingPerformance2GeVKF} illustrates that tracking efficiency improves when the \KF's covariance matrix accounts for \MS.
...
Figure~\ref{fig:kfHelixParametersResVsInvPt} shows the resolutions of the track parameters as a function of track $\frac{1}{\pT}$ in simulation for both of these \MS coefficients and for when \MS was not accounted for at all.
...
For both \MS coefficients, however, the \pt($\frac{1}{\pT}$) resolution in the range $3.8 \GeV < \pt < 5.5\GeVc$ ($0.18 1/\GeV < \pt < 0.26 1/\GeVc$) was worse than when \MS was not considered.
