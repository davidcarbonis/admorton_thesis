...
From equations~(\ref{eq:chi4}) and~(\ref{eq:chi5}) and the track derivatives as derived in~\cite{CMS_DN-14-043}, a software implementation of the linearised $\chi^{2}$ track fit algorithm was developed.
...
\caption{The post-\HT (top) and post-\KF (bottom) tracking efficiency distributions for tracks with $\pt > 2\GeVc$ for \ttbar events with a $<\text{PU}>$ of 200. The default configuration where only the number of \qpt columns are increased are shown in red and the configuration with the increased number of columns, HT cell merging and \KF state cuts optimisation shown in blue).
}
...
\begin{table}[htbp]
\topcaption {Track finding performance on simulated \ttbar events with a $<\text{PU}>$ of 200, after the \HT and the full chain have been considered for the configurations of only increasing just the number of \qpt columns (\emph{Default}),and also applying \HT cell merging and the optimised \KF state cuts (\emph{Optimised}).
}
...
\caption{The $\frac{\chi^{2}}{ndf}$ distribution as a function of $\frac{1}{\pT}$ for \ttbar events with a $<\text{PU}>$ of 200 after the full chain has been run. The distribution for when the \KF has not been modified to take \MS into account is given in red, for a constant \MS coefficient in black and a layer-dependent \MS coefficient in blue.
}
...
\caption{Distributions of \pt resolution, $\phi$ resolution, $z_{0}$ resolution and cot$\theta$ resolution determined for primary reconstructed tracks in simulated \ttbar events with a $<\text{PU}>$ of 200. The distribution for when the \KF has not been modified to take \MS into account is given in red, for a constant \MS coefficient in black and a layer-dependent \MS coefficient in blue.
}
