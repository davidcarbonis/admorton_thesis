...
These track fitting algorithms are based on linear fits in the \emph{r-$\varphi$/r-z} planes, one being a Linear Regression (LR) fit, which is further discussed in~\cite{TMTT_FLP}, and the other a $\chi^{2}$ fit.

The linearised $\chi^{2}$ fit makes use of residuals between the stubs and the seeded track that minimise $\chi^{2}$ of the fit in order to obtain improved parameters for the track candidate.
...
Eqn.~\ref{eq:chi4} gives a linear form which can be solved for $\delta h$ through Newtonian Iteration through updating the residuals with respect to the seeded track candidate.

\editComment{Worth showing track derivatives in full here, or in an appendix?}

Whilst the computation of these matrices in full would be non-trivial for FPGAs, 

several aspects making it suitable for FPGA implementation compared to global track fitting methods, namely the matrices:

%%internal cuts, multiple iterations

Despite the performance in simulation of the linear $\chi^{2}$ track fitter being comparable to both the \KF and \LR track fitters, consideration as to whether it can be implemented in hardware had to be given.
...
Following the December 2016 demonstrator review and the proposition of the \emph{tilted} barrel geometry, \editComment{what impact did the new geometry have?}, the modification of the \KF state uncertainty to take into account the impact of multiple scattering for low \pT tracks was explored.
This involved adding a contribution of the order of a milliradian, the anticipated amount of scattering, to the r-$\varphi$ error~\footnote{The error for the r-z plane is not modified as the effect of multiple scattering is considerably smaller hit position resolution in r-z }, in quadrature to the \KF.


%%% - additive constant of order (1 mrad)^{2} to inflate r-phi error
%%% - hit position resolution in r-z means there's no sensitivity to scattering in r-z
%%% - separate cutting of chi2 in each plane?
...
Given that an increased number of TFPs not desirable for economic reasons, the options for reducing the number of FPGA boards required are reducing the time-multiplex period from 18 and/or the resources required by each TFP so that it can be composed of fewer FPGAs.
The easiest means of reducing the resources required for the system is to decrease the amount of resources allocated to avoid truncation effects from the inability to transmit all stubs from the \GP to \HT and for the \HT to readout stubs within latency constraints. 

The increased resources and link bandwidths of next generation FPGAs are anticipated 
 for the Outer the time-multiplex period 
options for reducing number of TFPs: reduce TMUX, reduce resources (\ie spare processing time for truncation)
%and that the number of $\phi$ sectors will increase from 16 to 18, as each processing sector is sub-divided into two $\phi$ sub-sectors~\ref{subsubsec:GP}.
