...
The Track Finding Processor is(Fig.~\ref{fig:TFP}) consists of four self-contained components:
\begin{itemize}
...
\subsection{Linearised $\chi^{2}$ Track Fitter}
...
Both require a proceeding \emph{Seed Filter} (SF) stage to remove stubs in a HT cell which are inconsistent with a straight line in the \emph{r-z} plane to filter out both fake tracks and stubs incorrectly assigned to tracks.
These track fitting algorithms are based on linear fits, one being a Linear Regression (LR) fit with two independent fits in the \emph{r-$\varphi$/r-z} planes, which is further discussed in~\cite{TMTT_FLP}, and the other a $\chi^{2}$ fit.

\editComment{helix parameters?}
...
\editComment{Worth showing track derivatives in full here, or in an appendix?}

By performing repeated iterations of Eqn.~\ref{eq:chi4}, the fitted helix parameters can be repeatedly improved upon, albeit with diminishing returns as more applications are made (Fig~\ref{}).
With the stubs' residuals available 

As stubs incorrectly associated with a track have 

Kill tracks with large residuals ... 
One potential 
...
\editComment{Precision required for the elements?}
...
When considering the minimum parameter resolution required which would not considerably impact on the track reconstruction efficiency or helix parameter resolutions, .... \editComment{Find out how many are required}
