...
\item \textbf{level-1 trigger contributions} are require in order to maintain L-1 trigger performance. It has been shown that the performance of the L1 trigger will deteriorate in the high luminosity environment from both the rate increase and the reduced efficiencies of the L1 selection algorithms~\cite{CMSCollaboration:2015zni}.
...
Therefore the L1 bandwidth and latency will be increased (from 100\kHz to 750\kHz and from $3.2\mus$ to $12.5\mus$ respectively) and tracking information will be included in the L1 decision process to preserve and improve trigger performance.
...
Given that it will not be possible to read out every hit in the Outer Tracker for the L1 trigger with the LHC bunch crossing rate of 40\MHz, a novel design of a pair of closely-spaced silicon sensor layers, separated by a few mm, has been proposed.

Figure~\ref{fig:stubs} illusrates how these sensors, known as the \emph{\pT modules}~\cite{jjonespixel,markthesis}, are able to determine a pair of hits produced by a track are consistent with a track \pT above a configurable threshold (typically 2-3\GeV).


Assuming that a particle originates at the beam line

Figure~\ref{fig:stubs}(c) - how a larger sensor spacing is required in the endcap.

\begin{itemize}
\item Correlating closely spaced clusters between two sensor layers, separated by a few mm, allows discrimination of transverse momentum based on the particle bend in the CMS magnetic field, on the assumption that the particle originated at the beam-line. shown in figure~\ref{fig:stubs}(a).
\item The same transverse momentum corresponds to a larger distance between signals for a given sensor spacing.
\item A larger spacing is  Only tracks with \pT $> 2-3$\GeVc are transferred off-detector
\end{itemize}


are able to discriminate on a charged particle's \pT based on the local bend of the track within the magnetic field.
...
Hardware demonstrators for each of the three proposed L1 track finder projects were constructed to prove the feasibility of each approach, which were reviewed in 2016.
...
\section{A FPGA-Based Track Finder}\label{sec:TMTT}
TEXT HERE
...
Unlike the Phase-I Calorimeter Trigger, it is unfeasible to process the entire output of the Phase-II Outer Tracker in a single processor for a given time slice because of the limits imposed by the input and total bandwidth a single FPGA-based processor can handle.
...
% and a .pdf suffix will be assumed for pdflatex; or what has been declared
% via \DeclareGraphicsExtensions.
\caption{The four self-contained logical components of the Track Finding Processor, where each box (block) represents a single FPGA. The two FPGAs for the two detector octant sources and the sink FPGA and the optical links between all components are also shown.}
\label{fig:TFP}
\end{figure}
...
During the development of the \emph{TMTT} demonstrator system both before and following the 2016 review, the author was involved in a number of simulation studies, the more substantive of which are presented below.
...
Whilst the Kalman Filter was used as a track fitter for the 2016 hardware demonstrator, a Linear Regression (LR)\footnote{The LR fitter~\cite{TMTT_FLP} was developed as an alternative to the KF and exploits the fact that sufficiently high \pT tracks should form a straight line in the \emph{\rphi} and \emph{r-z} planes to perform independent fits in each plane.} algorithm and Linearised $\chi^{2}$ fitting algorithm were also explored.

The studies into a linearised $\chi^{2}$ track fit were initially motivated by the \emph{TMTT} project anticipating potential time and resource pressures in developing all the components of a complete track finder system that could be implemented in hardware due to the project being formed significantly after the other two L1 track finder projects.
