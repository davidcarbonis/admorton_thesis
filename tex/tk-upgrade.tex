...
As the \emph{TMTT} project was formed a significant amount of time (\editComment{Find out how much? Cannot recall off hand...}) after the other two L1 track finder projects, there was a pressing need to be able to perform precision helix parameter fits following the \HT using an algorithm which could could be conceivably implemented in hardware.
...
Whilst the decreased precision HT cells at low \pT recovers a proportion of the lost tracks, Fig.~\ref{fig:2GeVFlatEff}  also shows that the \KF's performance is significantly degraded for $\pT < 3 \GeV$ compared to $\pT > 3 \GeV$.
These additional low \pT track losses are the result of the \KF not modelling taking into account the increased uncertainty in the hit positions resulting from MS. 
This is illustrated by the distributions of $\chi^{2} \div \text{number of degrees of freedom}$ ($\frac{\chi^{2}}{ndf}$) as a function of $\frac{1}{\pT}$ in Fig.~\ref{fig:2GeVFlatChi2Ndf}, where $\frac{\chi^{2}}{ndf}$ is approximately flat near unity for high \pT (low $\frac{1}{\pT}$) but significantly increases ... 

\editComment{Finish detailed description once plots have been made - delayed as these plots apparently didn't exist for the flat geometry era! Need to backport and remake ...}
...
%%% - KF internal chi-squared cuts; would expect if MS was fully accounted for, then on average chi2/NDF = 1  everywhere. 

Due to the higher priority elements of the \emph{TMTT} demonstrator limiting the time and resources given to the robustness studies, studies into the incorporation of MS in the \KF were undertaken after the December 2016 demonstrator review.
As all the projects migrated from using the \textit{flat} barrel geometry to the \textit{tilted} barrel geometry at the start of 2017, all the following results are presented using the latter geometry.

%%%
As discussed in \ref{subsubsec:KF}, the Kalman state is iteratively updated by applying stubs and their associated uncertainties.
By including the effects of multiple scattering in t
%%%high energies = resolution error
...
With the particles involved having relativistic velocities (\ie $\beta c \cong 1$) and any scattering in the r-z plane being ignored, and te

, an initial conservative approximation for this multiple scattering contribution can be expressed as:

\begin{equation}
 $\theta_{rms} = \frac{const}{r_{track}}$.
\label{eq:scatter2}
\end{equation}
