...
Both the Linear Regression and Linearised $\chi^{2}$ fitting algorithms require a proceeding \emph{Seed Filter} (SF) stage to remove stubs in a HT cell which are inconsistent with a straight line in the \emph{r-z} plane to filter out both fake tracks and stubs incorrectly assigned to remove stubs in a HT cell which are inconsistent with a straight line in the \emph{r-z} plane to filter out both fake tracks and stubs incorrectly assigned to tracks.
...
%Appendix~\ref{app:chi2} provides the derivation of the individual track derivatives used by the algorithm.
Calculation of the derivatives for the barrel layer and endcap disk hits from this general form, including a correction factor for $\phi$ in the outer disks to account for the fact that these modules do not point directly towards the interaction point, are given in Appendix~\ref{app:chi2}.

\subsubsection{Track Helix Parameters}\label{subsubsec:helixParameter}
Tracks throughout this chapter are described using five helix parameters:

\begin{itemize}
\item \textbf{$\frac{1}{R}$} - the inverse radius of curvature of the track. As the magnetic field in the tracker is constant and the radius of curvature is proportional to the transverse momentum of the track, results are quoted in terms of $\frac{q}{\pT}$ (where q is the charge of the particle).
\item \textbf{$\phi{0}$} - the track direction at the point of closest approach to the interaction point.
\item \textbf{$z_{0}$} - the z position of the helix at the point of closest approach to the interaction point.
\item \textbf{$\tan (\lamda)$} - the tangent of the \emph{dip} angle, related to $\eta$ by $\tan (\lamda) = \sinh (\eta)$ and expressed in terms of $\eta$ for the results presented.
\item \textbf{$d_{0}$} - the \emph{impact parameter}, \ie the distance of the track vertex from the interaction point in the $x-y$ plane. The \HT and track fitting algorithms discussed all assume that all tracks originate at the interaction point.
\end{itemize}

\subsubsection{General Form of a $\chi^{2}$ Fit}\label{subsubsec:chi2maths}

Initially, assuming one has a track with at least two hits, the 

of each stub $s_{i}$, based on the track’s helix parameters $h$, are initially linearly expanded around the estimate of the helix parameters $h_{0}$:
...
which, in order to obtain $\delta h$ so that ... , is minimised:
...
Using the track derivatives derived from the general form of Eqn.~\ref{eq:chi4} and detailed in Appendix~\ref{app:chi2}, a floating point software implementation of the algorithm was developed.
This implementation was used in order to both validate the algorithm and to optimise its performance prior to producing a version 
...
The track finding efficiencies, the mean numbers of reconstructed tracks per event in the entire tracker, and the number of those tracks which are fakes or duplicates, are given using the efficiency definitions described in Chapter~\ref{TmttSimStudies}.
...
The track finding efficiencies, the mean numbers of reconstructed tracks per event in the entire tracker, and the number of those tracks which are fakes or duplicates, are given using the efficiency definitions described at the start of Chapter~\ref{TmttSimStudies}.
\editComment{better to quote non-integer numbers and fractions of duplicates/fakes?}.
}
...
% This increases column spacing.
  \addtolength{\tabcolsep}{1ex}
% This right-aligns numbers in column, but centers them under column title.
  \begin{tabular}{ccr@{\hspace{4ex}}r@{\hspace{4ex}}r@{\hspace{4ex}}{\hspace{4ex}}{\hspace{4ex}}{\hspace{4ex}}{\hspace{4ex}}}
   \hline
   \bf{Configuration} & \bf{Stage} & \bf{Efficiency [\%]} & \multicolumn{1}{r}{\bf{Mean \# of tracks}} & \multicolumn{1}{r}{\bf{\# of fakes}} & \multicolumn{1}{r}{\bf{\# of duplicates}}  \\
   \hline
...
Figure~\ref{fig:kfHelixParametersResVsInvPt} shows the resolutions of the four track parameters as a function of transverse momenta in simulation for tracks originating from the primary interaction in \ttbar events at \PU of 200 for  the both \MS coefficients investigated and for no consideration of \MS at all.
