...
The TFP uses it to find primary charged particles with \pT > 3\GeV in the \rphi plane. 
...
The LR fitter~\cite{TMTT_FLP} was developed as an alternative to the KF and exploits the fact that sufficiently high \pT tracks should form a straight line in the \emph{r-$\phi$} and \emph{r-z} planes to perform independent fits in each planes.
...
With the particles involved having relativistic velocities (\ie $\beta c \cong 1$) and scattering in the r-z plane ignored as the impact of multiple scattering is considerably smaller hit position resolution in r-z, the multiple scattering contribution in the r-$\phi$ plane can be expressed as:
...
During the comparative studies of these two approaches, it was observed in the results following the \DR stage that in contrast to the trend of the fraction of duplicates decreasing as \pT decreases, the fraction of duplications present increases above circa $3\GeV$, as shown in Fig~\ref{2GeVfracDups}.
...
Whilst the similar performance 
...
%This is not unexpected, given that a constant inflation in the r-$\phi$ plane hit resolution error is applied for each stub applied, whilst in reality the inflation would vary as a particle traverses material.
...
\item separate \KF $\chi^{2}$ cuts for the r-$\phi$ and r-z planes ... WHY?
