...
This decision was made as the \KF was capable of achieving a higher track finding efficiency with 100.0\% purity for matched tracks and $z_{0}$ and $cot(\theta)$ resolutions in the forward regions that were comparable to the expected offline resolution.
...
The studies were initially undertaken as part of the robustness studies required for the 2016 demonstrator review, which investigated the impact that lowering the track reconstruction \pT threshold had on the \HT.
...
Without applying further modifications, there is a considerable degradation in the track reconstruction efficiency in the range $2\GeVc < \pt < 2.7\GeVc$, due to these low momenta tracks being dominated by \MS.
...
As this feature was also present following the \HT stage, this implies that the \HT produces more duplicates at low \pT in the full precision cells.
The fraction of duplicate tracks produced where decreased precision \HT cells are used was well controlled, the \pT ($\frac{1}{\pT}$) threshold for the $2 \times 2$ merging of \HT cells was increased from $2.7\GeV$ ($0.37/\GeV$) to $3.5\GeV$ ($0.29/\GeV$).
An increase in the duplicate rate however, was observed for \pT ($\frac{1}{\pT}$) between $3.2\GeV$ ($0.31 /\GeV$) and $5\GeV$ ($0.20/\GeV$) in Figure~\ref{fig:2GeVfracDups} a decreased number of duplicates were produced below the \pt ($\frac{1}{\pT}$) threshold of $3.5\GeV$ ($0.29/\GeV$), which led to an overall reduction in the number of duplicates produced of about 2.8\%.
The observed increases in the duplicate rate observed in Figure~\ref{fig:2GeVfracDups} are yet to be fully understood, but are suspected to have arisen from resolution effects at the boundaries between the differently sized \HT cells given their proximity to the \pT threshold for the merging of \HT cells.
...
As shown in figures~\ref{fig:2GeVTiltEff} and~\ref{fig:kfHelixParametersResVsInvPt} and table~\ref{tab:trackFindingPerformance2GeVKF}, there are only small differences in the performance of the two \MS coefficients.
