...
The design 
%% constrained by l1 latency and cabling
...
%It is planned to increase the L1 trigger's bandwidth and latency from 100\kHz to 750\kHz and from $3.2\mus$ to $12.5\mus$ respectively.
%Out of the total $12.5\mus$ L1 latency, $\approx 3.5\mus$ is allocated for the correlation of tracks with trigger primitives from the calorimeters and muon systems, $1\mus$ for the propagation of the L1 decision to the front-end buffers and $3\mus$ for a safety margin~\cite{CMS_Upgrade_TP}.
%From the remaining $\approx 5\mus$, $\approx 1\mus$ is required for generation, packaging and transmission of stubs from the tracker front-end electronics to the Data, Trigger and Control (DTC) system, leaving $\approx 4\mus$ available for the reconstruction of tracks from data arriving at the DTC, as shown in Fig.~\ref{fig:dataFlow}.
