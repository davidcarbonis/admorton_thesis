...
The current proposed layout of the Phase-II Outer Tracker, referred to as the \emph{tilted barrel} geometry, is depicted in the upper diagram in Fig.~\ref{fig:trackerlayout}, and a previous proposal, referred to as the \emph{flat barrel} geometry, is shown in the lower diagram~\cite{CMS_Upgrade_TP}.
...
Due to the higher priority elements of the \emph{TMTT} demonstrator limiting the time and resources given to the robustness studies, studies into the incorporation of multiple scattering in the \KF were undertaken after the December 2016 demonstrator review.
Following the review, 
All the following results presented were produced using the \textit{tilted} barrel geometry\footnote{Whilst the As all the projects migrated from using the \textit{flat} barrel geometry to the } barrel geometry at the start of 2017, all the following results are presented}.
Initially the form  
...
With the particles involved having relativistic velocities (\ie $\beta c \cong 1$) and any scattering in the r-z plane being ignored as the effect of multiple scattering is considerably smaller hit position resolution in r-z, an initial conservative approximation for this multiple scattering contribution can be expressed as:
...
Therefore, by considering the deflection of the charged particle tracks due to multiple scattering as a, the \KF
%%%

With this simplified expression of the multiple scattering angle
Two ideas:
...
\begin{itemize}
\item determining suitable coefficients as functions of both \pT and $\eta$ experimentally through simulation to account for the increased hit resolution error of a stub based on the amount of material that has been previously traversed.
