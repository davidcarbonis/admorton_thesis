...
Therefore, it is preferable to measure the stub coordinates relative to the point where it crosses a cylinder in the $x--y$ plane of radius $T$.
This is because if an appropriate choice of $T$ is used, the transformation of $r \rightarrow r_{T}$ and $\phi \rightarrow \phi_{T}$ increases the size of Hough Transform phase space used>
This allows for more precise measurements of the intersection point to be made and consequently fewer fake tracks and duplicates are found.
...
As these incorrectly associated stubs will negatively impact the accuracy of the fitted track parameters and it is preferable to discard fake tracks, a \KF 
...
In addition to being the optimal linear filter and, in certain circumstance, the optimal non-linear filter, the \KF has several aspects that make it more suitable for FPGA implementation compared to global track fitting methods~\cite{Fruhwirth:1987fm}:

\begin{itemize}
\item {The matrices involved are small and are their size is independent of the number of measurements, minimising the logic required to implement them;}
...
The \KF begins with an estimate of the track parameters and their covariance matrix (containing the measurement uncertainties) from the \HT array and $\eta$-$\phi$ segment assignment, known as the \emph{state}.
Stubs are iteratively added to the predicted state in order to produced an updated state estimate formed of weighted combination of the predicted state and the measurement.
This weighting, known as the \emph{Kalman gain}, is derived from the relative uncertainties of the predicted state and measurement and is used to control how the state's track parameter are updated.
...
It can also be configured to take into account and skip missing missing layers when the expected stub is either missing or deemded to be incorrectly associated with the track.
