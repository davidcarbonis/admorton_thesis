...
This is due to a bug being discovered in the final version of the TMTT code for the flat geometry used to produce these results, which was not present during the development of this algo.
...
The larger stats (~1000 to 5000 events) runs will be done prior to final draft.}
...
In contrast to the floating point algorithm, the ``approximated'' maths version of the algorithm provides noticeably worse resolutions for \qpt and $\phi_{0}$ by approximately a factor of two.
The resolutions obtained for \qpt and $\phi_{0}$ endcaps and for $\eta$ and $z_{0}$ across the tracker are broadly comparable between the two approaches.

\editComment{Elaborate on resolutions further once the higher stats results are in - can make statements with much greater confidence! \ie explain why the worse barrel resolution for \qpt and $phi_{0}$ is acceptable - as my notes and memory does not correspond with this!
How does it compare to offline resolutions?}
...
% This right-aligns numbers in column, but centers them under column title.
  \begin{tabular}{ccr@{\hspace{4ex}}r@{\hspace{4ex}}r@{\hspace{4ex}}r@{\hspace{4ex}}r@{\hspace{4ex}}r@{\hspace{4ex}}}
   ...
   (floating point) & & & & & \\
   \hline
   $\chi^{2}$+DR & 94.8 & 85.6 & 89.1 & 15.2 & 12.5 \\  
   (approximated) & & & & & \\   
%   \hline
...
As shown in table~\ref{tab:chi2_exactVsApprox}, in \ttbar events at \PU of 200, a significant proportion of the tracks reconstructed by the linearised $\chi^{2}$ fit either are fake tracks or are matched tracks which contain at least one incorrect stub.
...
This concept was revisited however, when considering how to remove stubs which are either incorrectly associated with a matched track or belong to a fake track.
Such ``fake'' stubs are expected have larger residuals than those which are correctly associated and given that the residuals of each stub were known following a fit.
Therefore if the stub with the largest residual exceeded a suitable cut, it was removed from the track and the track was refitted with its reduced collection of hits. 
During the optimisation of this cut, it was found that this approach had the potential to leave a track with fewer than the minimum of four stubs required for a track candidate to be considered, thus potentially discarding a matched track by mistaken.
To avoid the discarding of genuine tracks whilst retaining the benefits of improving the quality of the tracks reconstructed whilst removing fake tracks, a looser residual cut was applied for tracks only comprised of four stubs.
Considering the removal of stubs occurs for each of the fitting iterations except for the final iteration, as by definition the final track fit has to occur on final collection of hits considered.

Table\ref~{tab:chi2-exactVsApprox} shows that whilst the tracking efficiency with no purity requirement

losses considerably impacted as the stub removal does mistakenly 
...
\editComment{Note that additional fitting iterations did not further improve on eff, or res}
\editComment{Better to show comparison between KF and it1 and it4? For the sake of clarity?}
...
% This right-aligns numbers in column, but centers them under column title.
  \begin{tabular}{ccr@{\hspace{4ex}}r@{\hspace{4ex}}r@{\hspace{4ex}}r@{\hspace{4ex}}r@{\hspace{4ex}}r@{\hspace{4ex}}r@{\hspace{4ex}}}
   ...
   \hline
   $\chi^{2}$ & 1 & 94.8 & 85.6 & 89.1 & 15.2 & 12.5 \\  
   & 2 & 93.4 & 90.5 & 74.2 & 6.6 & 9.0 \\
   & 3 & 92.6 & 90.4 & 71.7 & 5.3 & 8.3 \\
   & 4 & 92.5 & 90.3 & 71.5 & 5.2 & 8.2 \\
%   & 5 & 92.2 & 89.5 & 71.4 & 5.5 & 8.0 \\
%   & 10 & 92.2 & 89.5 & 71.4 & 5.5 & 8.0 \\
%   & 15 & 92.2 & 89.5 & 71.4 & 5.5 & 8.0 \\
   \hline
...
Following these preliminary studies of a linearised $\chi^{2}$ track fit, a comparison was made between it and the \KF.
