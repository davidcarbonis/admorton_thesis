...
The other two proposasls are all-FPGA approaches, one using a fully Time-Multiplexed Track (\emph{TMTT}) finder which uses the Hough Transform to identify track candidates~\cite{TMTT_JINST} and one using a ``road search'' (\emph{tracklet}) algorithm to reconstruct tracks~\cite{tP2TrackerTDR,racklet}.
...
In this section the architecture and components of the \emph{TMTT} Track Finding Processor are discussed. 
...
\item {\bf Kalman Filter (KF):} A track filtering and fitting stage which removes fake tracks, stubs that are incorrectly associated to a track and precisely fits helix parameters.
...
In this case, the \HT describes a point $(x,y)$ in real space as a straight line with a gradient and intercept ($m,q$) in parameter space known as \emph{Hough}-space.
Conversely, a point in the \emph{Hough}-space corresponds to a straight line in real space. 
Therefore, a straight line corresponding to a set of points in real-space is given by the intersect of a set of lines corresponding to the real-space points in \emph{Hough}-space.

The \HT is used by the TFP to find the tracks of charged particles with $\pT > 3\GeV$ in the \rphi plane, which has the best resolution, independently for each $\eta$-$\phi$ sub-sector within each processing octant.

The radius of curvature, $R$ (cm), for a charged particle's trajectory can be described as a function of the particle's \pT, charge $q$ and of the homogeneous magnetic field, $B$, that has bent its trajectory:
...
A stub with coordinates ($r$,$\varphi$) is related to $R$ by:
...
If energy losses are neglected from processes such as multiple scattering and Bremsstrahlung, the position of the stubs will be compatible with the trajectory described by (\ref{eq:stub_R}) (\ie $R$ can be assumed to be constant).

Therefore, Equations~(\ref{eq:R}) and~(\ref{eq:stub_R}) can be combined to describe how a stub's position in ($r, \varphi$) can be transformed into a line in the (\qpt,$\phi$) parameter space:
...
As illustrated in figure~\ref{fig:HT}, if a particle produces multiple stubs, the intersection of the corresponding stub lines in Hough-space can be used to identify a track candidate.
...
As the line gradients in Hough-space are given by the radii of the stubs, they will always be positive.
Therefore, it is preferable to measure the stub coordinates relative to the point where it crosses a cylinder of radius $T$.
Through an appropriate choice of $T$, the transformation of $r \rightarrow r_{T}$ and $\phi \rightarrow \phi_{T}$ increases the size of Hough Transform phase space used, allowing for more precise measurements of the intersection point to be made.
This results in fewer fake tracks and duplicates being found.
The optimal value of $T$ was determined to be 58 cm.
...
Following from the \HT is highly efficient method of finding track candidates, over half the track candidates are considered to be \emph{fake} or contain at least one incorrectly associated stub.
A Kalman Filter was used to filter the tracks candidates whilst precisely fitting their track parameters.

In addition to the advantages of the Kalman filter for track reconstruction discussed by Fr{\"u}hwirth in \cite{Fruhwirth:1987fm}, the algorithm has several aspects making it suitable for FPGA implementation compared to global track fitting methods:

\begin{itemize}
\item {the matrices involved are small and are their size is independent of the number of measurements}
...
A Kalman Filter uses both the coarse \rphi helix parameters from the \HT and ... as the initial variables for filtering the track candidates and precisely fitting helix parameters to them.
Ideally a \KF would be used

The coarse \rphi helix parameters that are output by the \HT are used as the initial variables for track finding, with the $\eta$-$\phi$ segment assignment also providing a good seed value.

Starting from an initial estimate , or \textit{state}, of the track parameters and their uncertainties, stubs are iteratively applied to update the state following the Kalman formalism.

The \KF iteratively applies stubs


The initial estimates $\chi^2$ value and other status information are updated by the KF iteratively applying stubs , decreasing the uncertainty in the state. 
...
This means that while events which a large number of track candidates, such as dense jets, will always be read out, they be only partially filtered.

Further discussion of the mathematics and implementation of online track reconstruction using Kalman Filters on FPGAs can be found in~\cite{TMTT_JINST,SSummers}.
...
All of the results discussed were obtained using simulated \ttbar events with a $<\text{PU}>$ of 200 events
and use a common set of definitions, as defined in Section~\ref{subsec:helixParameter}.
The results from track fitting algorithms discussed use digitised output from the \HT.
...
The development of a Kalman Filter was motivated by it being the optimal method of filtering a linear system, making it ideally suited removing incorrectly assigned stubs from tracks and the removal of fake track candidates.
As the \KF provided the best perfect track reconstruction efficiency and fake track candidate rejection rate out of the three fitting algorithms explored, it was selected as the baseline fitter for the 2016 review.

A Linear Regression (LR) track fitting algorithm was developed as an alternative to the KF~\cite{TMTT_FLP}.
As high \pT tracks should form a straight line in the \emph{\rphi} and \emph{$r-z$} planes, the the linear nature of the maths of a LR fit makes it well suited to perform independent fits in each plane using minimal latency and resources.

A linearised $\chi^{2}$ track fit was the first fitting algorithm to be studied by the \emph{TMTT} project.
These studies were motivated by the need to quickly develop and implement a track fitting algorithm which could be used with 

which could be quickly implemented given the time and resource pressures in developing a hardware demonstrator for a complete track finder system for the 2016 review.
...
The general form of the $\chi^{2}$ fit and the derivation of the track derivatives required by the algorithm were provided in a private communication~\cite{CMS_DN-14-043} and were used to produce a \emph{TMTT} implementation of it.
...
A discussion of the development and outcomes of the software implementation of the fitting algorithm are given in Section~\ref{subsubsec:chi2software}.
...
All the results presented here involve the use of the \emph{Seed Filter} (SF) stage that was run following the \HT stage for both the Linear Regression and Linearised $\chi^{2}$ fitting algorithms.
...
\subsubsection{$\chi^{2}$ Track Fitter Results}\label{subsubsec:chi2software}
...
From Equations~(\ref{eq:chi4}) and~(\ref{eq:chi5}) and the track derivatives as given in~\cite{CMS_DN-14-043}, a software implementation of the linearised $\chi^{2}$ track fit algorithm was developed.

The strategy behind the development of this track fitting algorithm intiially involved 

debugging and optimising the floating point maths implementation to give optiminal performance which was comparable to 
...
\begin{table}[htbp]
\topcaption {Track finding performance on simulated \ttbar events with a $<\text{PU}>$ of 200 events, after the \HT and the full chain for  both the exact floating point and approximated calculations of the track derivatives used by the $\chi^{2}$ track fit.
}
...
% This right-aligns numbers in column, but centers them under column title.
 \begin{tabular}{cccccc}
   ...
   \hline
%   $\chi^{2}$+no DR & 95.2 & 8 & 147.3 & 4.6 & 52.5 \\
%   (floating point) & & & & & \\
...
Table~\ref{tab:chi2-exactVsApprox} shows how the tracking performance between the floating point maths and ``approximated'' maths implementations of the algorithm compare against each other and from the raw track finding output from the \HT.
...
Figure~\ref{fig:chi2HelixParametersResVsEtaApproxVsExact} shows that resolutions of the four track parameters as a function of $\eta$ from primary tracks in \ttbar events with a $<\text{PU}>$ of 200 events for both mathematical implementations of the algorithm.
...
Therefore, in order to decrease the fake rate and increase the matched track purity the stub with the worst/largest residual is compared against a configurable threshold.
...
\begin{table}[htbp]
\topcaption {Track finding performance on simulated \ttbar events with a $<\text{PU}>$ of 200 events, for one to four fitting iterations, $N_{It}$, of the $\chi^{2}$ track fit and for the \KF. 
...
Table~\ref{tab:chi2_iterations} illustrates how the tracking performance of the linearised $\chi^{2}$ fitter improves with just one additional fitting iteration (removal of a stub), with the fake rate being halved and the purity of the matched tracks noticeably increasing.
...
As the number and width of \qpt \HT columns used varies from the standard fixed number and size of columns typically used, thus varying the \pT resolution available, results are expressed as a function of 1/\pT instead of \pT.
...
\caption{The post-\HT (top) and post-\KF (bottom) tracking efficiency distributions for tracks with $\pt > 2\GeVc$ for \ttbar events with a $<\text{PU}>$ of 200 events. The default configuration where only the number of \qpt columns are increased are shown in red and the configuration with the increased number of columns, HT cell merging and \KF state cuts optimisation shown in blue).
}
...
\begin{table}[htbp]
\topcaption {Track finding performance on simulated \ttbar events with a $<\text{PU}>$ of 200 events, after the \HT and the full chain have been considered for the configurations of only increasing just the number of \qpt columns (\emph{Default}),and also applying \HT cell merging and the optimised \KF state cuts (\emph{Optimised}).
}
...
The impact of \MS on the \KF is further illustrated by figure~\ref{fig:2GeVFlatChi2Ndf}, which shows the distributions of $\chi^{2}$ per number of degree of freedom ($\frac{\chi^{2}}{ndf}$) as a function of $\frac{1}{\pT}$ for genuine tracks produced by the \KF.
...
%\includegraphics[width=\textwidth]{figs/tk-upgrade/results-lowPtTracking/kfTrackingEffVsInvPtFlatGeometry_5000.pdf}
\caption{Plots showing tracking efficiency as a function of $\frac{1}{\pT}$ for \ttbar events with a $<\text{PU}>$ of 200 events after the full chain has been run, where the \KF has not been modified to take \MS into account (red), a constant coefficient for \MS is used (black) and a layer dependent coefficient for \MS is used (blue).
}
...
\begin{table}[htbp]
\topcaption {Track finding performance on simulated \ttbar events with a $<\text{PU}>$ of 200 events, after the full demonstrator chain for the three differing \KF configurations where \MS is not considered ($k = 0$), a constant \MS coefficient (const $k$) is used and a layer dependent \MS coefficient ($k(layer)$)is used. 
...
\caption{The $\frac{\chi^{2}}{ndf}$ distribution as a function of $\frac{1}{\pT}$ for \ttbar events with a $<\text{PU}>$ of 200 events after the full chain has been run. The distribution for when the \KF has not been modified to take \MS into account is given in red, for a constant \MS coefficient in black and a layer-dependent \MS coefficient in blue.
}
...
As shown in figures~\ref{fig:kfHelixParametersResVsInvPt}--~\ref{fig:2GeVTiltEff} and table~\ref{tab:trackFindingPerformance2GeVKF}, there are only small differences in the performance of the two \MS coefficients.
...
\caption{Distributions of \pt resolution, $\phi$ resolution, $z_{0}$ resolution and cot$\theta$ resolution determined for primary reconstructed tracks in simulated \ttbar events with a $<\text{PU}>$ of 200 events. The distribution for when the \KF has not been modified to take \MS into account is given in red, for a constant \MS coefficient in black and a layer-dependent \MS coefficient in blue.
}
\label{fig:kfHelixParametersResVsInvPt}
\end{figure}

\subsubsection{Outcome of tracking at low transverse momenta studies}\label{subsubsec:2GevConclusions}
