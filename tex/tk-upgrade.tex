...
The Hough Transform algorithm is a widely used method of detecting geometric features in digital image processing~\cite{HT} and is used by the TFP to reconstruct primary charged particles tracks in the \rphi plane from a set of stubs.

The \HT's suitability for this task is illustrated by considering the simplest use of the \HT algorithm: the detection of straight lines.
In this case, the \HT describes a point $(x,y)$ in real space as a straight line described by a gradient, $m$, and intercept, $q,$.
Conversely, a point in the \emph{Hough}-space corresponds to a straight line in real space. 



Given that the trajectory of a charged particle with a sufficiently high \pT ($> 2-3\GeV$) forms a straight line in the \rphi plane, the \HT is well suited to identifying tracks from a set of hits.
...
% and a .pdf suffix will be assumed for pdflatex; or what has been declared
% via \DeclareGraphicsExtensions.
\caption{Illustration of the Hough Transform. The left-hand side shows the trajectory of a single charged particle in one quarter of the tracker barrel in the $x-y$ plane. The right-side illusrates The six stubs which the particle produces are represented as dots in real space, but as lines 	in Hough-space on the right-hand side}
\label{fig:HT}
\end{figure}

The \HT is used by the TFP to find charged particles tracks with $\pT > 3\GeV$ in the \rphi plane, which has the best resolution, independently for each $\eta$-$\phi$ sub-sector within each processing octant.

The trajectory of a charged particle through the tracker is bent by the homogeneous 3.8T magnetic field, $B$ (T).
The radius of curvature of this trajectory, $R$ (cm), can be described as a function of the particle's \pT and charge $q$:
...
By neglecting energy losses such as through multiple scattering and only considering primary tracks from or near the primary interaction point (other such tracks are not typically relevant to the L-1 trigger), $R$ can be assumed to be constant.
As $R$
For this assumption and for large $R$, a stub with coordinates ($r$,$\varphi$) is related to $R$ by:

\begin{equation}
\frac {r}{2\,\pt} = \sin\left(\varphi-\phi\right) = \varphi-\phi\;
\label{eq:stub_R}
\end{equation}

where $\phi$ is the angle of the track in the transverse plane at the origin~\cite{markthesis}. 
For large \pT ($> 3\GeV$) and thus large $R$, the small angle approximation can be used. 

Combining Equations~(\ref{eq:R}) and (~\ref{eq:stub_R}), the key formula showing the transformation from stub positions to straight lines in the track parameter plane (Hough-space) can br produced:
...
The point of intersection of these lines in Hough-space would therefore correspond to a circle in the \rphi plane that is consistent with the primary interaction point and all stubs involved.

As the line gradients in Hough-space are given by the radii of the stubs, they will always be positive.
Therefore, it is preferable to measure

the stub radius is transformed to $r_{58} = r - 58$ cm in order to utilise a larger area of the Hough Transform phase space, leading to fewer fake (a found track that does not match to a simulated particle) and duplicated tracks.
The choice measuring $\phi$ at the point it crosses a cylinder of 58 cm 

Given that $R$ for the lowest \pT track (3\GeV) to be considered is greater than the outer radius of the tracking detector ($r$ = 1.2 m), all relevant particles are expected to traverse at least six barrel layers or endcap disks. 
...
This threshold can be further reduced to four layers to account for the reduced geometric coverage between $0.89 < \eta < 1.16$ or for dead detector layers or disks without significantly increasing the volume of data to be considered.
...
Given that in simulation over half the track candidates from the HT are considered to be \textit{fake} or to contain at least one stub associated with another particle, 


a Kalman Filter is used to both remove these incorrect stubs and reject fake tracks. 
...
The development of each of these algorithms

While the \HT produces a large number of incorrectly reconstructed tracks and stubs incorrectly associated with a track
The development of a Kalman Filter track fitter was motivated by the \HT  property of being the optimal method of filtering a linear system.

As the \KF provided the best perfect track reconstruction efficiency and fake track rejection rate out of the three fitting algorithms explored, it was selected as the baseline fitter for the 2016 review.

A Linear Regression (LR) track fitting algorithm was developed as an alternative to the KF~\cite{TMTT_FLP}.
As sufficiently high \pT tracks should form a straight line in the \emph{\rphi} and \emph{r-z} planes, the the linear nature of the maths of a LR fit makes it well suited to quickly perform independent fits in each plane.

The studies of the linearised $\chi^{2}$ track fit were initially motivated by the \emph{TMTT} project needing a track fitting algorithm which could be quickly implemented given the time and resource pressures in developing a hardware demonstrator for a complete track finder system for the 2016 review.
