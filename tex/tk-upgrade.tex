...
\editComment{WARNING - all current results were made WITHOUT SEED FILTER. Emergency reprocessing ongoing.}
...
By defining the matrices $D_{ij} = \frac{\partial f_{i}}{\partial h_{k}}$ and $M = D^{T} V^{-1} D$, Eqn.~\ref{eq:chi3} can be rewritten and solved for $\delta h$:
...
\editComment{Results in this subsub section have been made with 100 ttbar+pu200 events. Larger stats (~5000) runs will be done prior to final draft.}

From Eqns~\ref{eq:chi4,eq:chi5} and the track derivatives $D_{ij}$ derived in Appendix~\ref{app:chi2}, a software implementation of the linearised $\chi^{2}$ track fit algorithm was developed.

Following the validation of a floating point version of the algorithm, a version was developed using approximated calculations of the track derivatives in order to simply (and reduce resources required for) any future derivative tabulation by reducing the number of variables that it would depend on.
...
Both versions produce comparable results for the mean tracking efficiency, the mean number of tracks reconstructed  and what proportion of them are with or fake and duplicate tracks, indicating that the approximations made are broadly acceptable.

It is also useful to consider the quality of the reconstructed tracks in terms of the resolutions obtained for their helix parameters.
Figure~\ref{fig:chi2HelixParametersResVsEtaApproxVsExact} shows the resolutions of the four track parameters produced by both version of the algorithm as a function of $\eta$ in simulation for tracks originating from the primary interaction in \ttbar events at \PU of 200.
In contrast to the floating point algorithm, the ``approximated'' maths version of the algorithm is noticeably poorer 

\editComment{Elaborate on resolutions}

\begin{table}[htbp]
\topcaption {Track finding performance on simulated \ttbar events at a \PU of 200, after the \HT and the full chain for  both the floating point and approximated calculations of the track derivatives used by the $\chi^{2}$ track fit.
...
\begin{figure}[htb]
\centering
\includegraphics[width=0.47\textwidth]{figs/tk-upgrade/results-chi2fitter/qOverPtResVsEta_It1_ApproxVsExact.pdf}
\includegraphics[width=0.47\textwidth]{figs/tk-upgrade/results-chi2fitter/phi0ResVsEta_It1_ApproxVsExact.pdf}
\\
\includegraphics[width=0.47\textwidth]{figs/tk-upgrade/results-chi2fitter/z0ResVsEta_It1_ApproxVsExact.pdf}
\includegraphics[width=0.47\textwidth]{figs/tk-upgrade/results-chi2fitter/etaResVsEta_It1_ApproxVsExact.pdf}
\caption{
...
As shown in table~\ref{tab:chi2_exactVsApprox}, in \ttbar events at \PU of 200, a significant proportion of the tracks reconstructed by the linearised $\chi^{2}$ fit are fake tracks, approximately 50\%, and the majority of the matched tracks reconstructed contain at least one incorrect stub, approximately 60\%.

Running multiple iterations of the fitting algorithm had been previously been considered in the context of improving upon the resolution of the fitted track's helix parameters, but had been shown to only produce rapidly diminishing returns with each new iteration.

When 

Given that stubs which are either incorrectly associated with a track or belong to a fake would be expected have larger residuals than those which are correctly associated, once a fit has been performed and the residuals of each stub are known, 

When considering that following a fit


 each additional iteration of the fitting algorithm yields diminishing improvements, the residuals associated to each stub were considered following each fitting iteration.
...
During the optimisation of the cut, it was found that this approach had the potential to leave a track with fewer than the minimum of four stubs required for a track candidate to be considered, potentially leading to the mistaken discarding of a matched track.
To avoid the discarding of genuine tracks whilst retaining the benefits of improving the purity of the tracks and the resolutions of their helix parameters and the removal of fake tracks, a looser residual cut was applied for tracks only comprised of four stubs.

Table\ref~{tab:chi2-exactVsApprox}
...
% This right-aligns numbers in column, but centers them under column title.
  \begin{tabular}{ccr@{\hspace{4ex}}r@{\hspace{4ex}}r@{\hspace{4ex}}r@{\hspace{4ex}}r@{\hspace{4ex}}r@{\hspace{4ex}}r@{\hspace{4ex}}}
   ...
   \hline
   $\chi^{2}$ & 1 & 96.9 & 32.3 & 243.3 & 50.6 & 22.1 \\  
   & 2 & 95.6 & 52.6 & 166.4 & 39.5 & 22.0 \\
   & 3 & 94.1 & 67.2 & 123.9 & 27.9 & 21.9 \\
   & 5 & 92.1 & 81.7 & 94.3 & 14.9 & 20.9 \\
   & 10 & 91.1 & 86.9 & 87.2 & 11.2 & 20.0 \\
   & 15 & 90.1 & 86.0 & 87.7 & 11.9 & 20.1 \\
   \hline
   KF+DR & - & 94.3 & 94.3 & 81.9 & 20.9 & 4.7 \\
   \hline
...
When considering the performance of the linearised $chi^{2}$ fitter compared to that of the \KF,
...
%=========================================================================
%                    GENERAL FITTING SUMMARY FOR: ChiSquared4ParamsApprox
%Number of fitted track candidates found per event = 87.2322 +- 0.2487
%                            with mean stubs/track = 5.5493
%Fraction of fitted tracks that are fake = 0.1124
%Fraction of fitted tracks that are genuine, but extra duplicates (post-cut) = 0.1999
%Algorithmic fitting efficiency (post-cut) = 79464/87249 = 0.9108 +- 0.0010
%Perfect algorithmic fitting efficiency(post-cut) = 75821/87249 = 0.8690 +- 0.0011 (no incorrect hits)
%=========================================================================
%                    GENERAL FITTING SUMMARY FOR: ChiSquared4ParamsTrackletStyle
%Number of fitted track candidates found per event = 86.7218 +- 0.2462
%                            with mean stubs/track = 5.5887
%Fraction of fitted tracks that are fake = 0.1146
%Fraction of fitted tracks that are genuine, but extra duplicates (post-cut) = 0.1874
%Algorithmic fitting efficiency (post-cut) = 79895/87249 = 0.9157 +- 0.0009
%Perfect algorithmic fitting efficiency(post-cut) = 76505/87249 = 0.8769 +- 0.0011 (no incorrect hits)
%=========================================================================
%
% 5 iterations
%=========================================================================
%                    GENERAL FITTING SUMMARY FOR: ChiSquared4ParamsApprox
%Number of fitted track candidates found per event = 94.3022 +- 0.2845
%                            with mean stubs/track = 5.9634
%Fraction of fitted tracks that are fake = 0.1490
%Fraction of fitted tracks that are genuine, but extra duplicates (post-cut) = 0.2086
%Algorithmic fitting efficiency (post-cut) = 80486/87359 = 0.9213 +- 0.0009
%Perfect algorithmic fitting efficiency(post-cut) = 71394/87359 = 0.8172 +- 0.0013 (no incorrect hits)
%=========================================================================
%                    GENERAL FITTING SUMMARY FOR: ChiSquared4ParamsTrackletStyle
%Number of fitted track candidates found per event = 93.9226 +- 0.2829
%                            with mean stubs/track = 5.9905
%Fraction of fitted tracks that are fake = 0.1516
%Fraction of fitted tracks that are genuine, but extra duplicates (post-cut) = 0.1975
%Algorithmic fitting efficiency (post-cut) = 80941/87359 = 0.9265 +- 0.0009
%Perfect algorithmic fitting efficiency(post-cut) = 71984/87359 = 0.8240 +- 0.0013 (no incorrect hits)
%=========================================================================
%
% 3 iterations
%=========================================================================
...
%   \hline
%    Merge & \bf{HT}     &  94.6 &  799  &  324 &   317 \\  
%    & \bf{Full chain}     &  89.8 &  206   &  53  &     8 \\      
    \hline
...
% This right-aligns numbers in column, but centers them under column title.
  \begin{tabular}{ccr@{\hspace{4ex}}r@{\hspace{4ex}}r@{\hspace{4ex}}r@{\hspace{4ex}}r@{\hspace{4ex}}r@{\hspace{4ex}}r@{\hspace{4ex}}}
   ...
   \bf{Configuration} & \bf{Stage} & \bf{Efficiency [\%]} & \multicolumn{1}{r}{\bf{Mean \# of tracks}} & \multicolumn{1}{r}{\bf{\# of fakes}} & \multicolumn{1}{r}{\bf{\# of duplicates}}  \\
   \hline
     & \bf{HT}     &  96.2 &  752  &  212 &   356 \\  
   No \MS  & \bf{Full Chain}     &  93.6 &  216   &  29  &     20 \\      
   \hline
   const $k$  & \bf{Full Chain}     & 90.0 &  222   &  22  &     31 \\      
   \hline
    $k(layer)$ & \bf{Full Chain}     &  94.2 &  222   &  24  &     27 \\  
   \hline
