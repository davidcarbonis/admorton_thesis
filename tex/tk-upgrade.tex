...
One uses Associative Memory (\emph{AM}) ASICs for track finding and FPGAs for track fitting, and the other two all-FPGA approaches, one using a fully Time-Multiplexed approaching using the Hough Transform to identify track candidates (\emph{TMTT}) and one using a ``road search'' (\emph{tracklet}) algorithm to reconstruct tracks.
