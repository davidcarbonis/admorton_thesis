...
A Kalman Filter was used to filter and precisely fit the tracks candidates produced by the \HT.
While 
WHAT DOES IT DO
...
Following from the \HT is highly efficient method of finding track candidates, over half the track candidates are considered to be \emph{fake} and over half the genuine candidates contain at least one incorrectly associated stub.
...
% and a .pdf suffix will be assumed for pdflatex; or what has been declared
% via \DeclareGraphicsExtensions.
\caption{An illustrated example of the \KF filtering procedure for a track candidate in the \rz plane of the barrel. 
}
\label{fig:KF}
\end{figure}
...
All of the results discussed were obtained using simulated \ttbar events with a $<\text{PU}>$ of 200 events and use digitised output from the \HT.
The set of metrics used to evaluate the performance of the proposed track finder systems are used for the results presented and are described in Section~\ref{subsec:definitions}.
...
Initially this implementation used exact floating point mathematics in order to validate that the algorithm had been correctly implemented.
