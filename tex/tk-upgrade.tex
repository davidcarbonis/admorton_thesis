...
\item {\bf Hough Transform (HT):} A highly parallelised initial coarse track finding that identifies track candidates that are consistent with a track in the \rphi plane. 
...
While the \KF is the optimal linear filter and, in certain circumstance, the optimal non-linear filter, it also has several aspects that make it more suitable for FPGA implementation compared to global track fitting methods~\cite{Fruhwirth:1987fm}:
