...
\item \textbf{layer-dependent coefficient:} the coefficient used depends on the layer ID (\ie the layer/disk in which the stub was found) in order to take into account the impact of repeated scattering from passing through multiple layers increasing the uncertainty associated with the hit position.
\end{itemize}
...
A process noise term was added to the \KF to describe the increased hit position uncertainty in the \rphi plane resulting from \MS.
Two different \MS terms were evaluated, one solely as a function of the track's \pT and the other as a function of the track's \pT and the tracker layer/disk the current stub was found in.
Both noise terms considered produced comparable results and were shown to not only improve the track reconstruction efficiency of the system, but also to account for the dominant source of uncertainty in the track measurements.
This performance could be further improved by using a process noise term that better accounts for the amount of material traversed and by establishing the cause behind the increased production of duplicate tracks observed near the boundary between normal and reduced precision \HT cells.
