...
While the \HT is highly efficient at finding genuine tracks, it was found in simulation that over half of the genuine tracks found contain at least one incorrectly associated stub and over half of the tracks candidates did not correspond to a genuine track (\ie were fake).
Given that these incorrectly associated stubs will negatively impact the accuracy of the fitted track parameters and  

As the presence of incorrectly associated stubs will  and 
As it is desirable to filter out tracks without a significant loss 

While it would be impractical to use a \KF to find tracks instead of the \HT , it 
A Kalman Filter was used to filter out and precisely fit the tracks candidates produced by the \HT.



In addition to the advantages of the Kalman filter for track reconstruction discussed by Fr{\"u}hwirth in~\cite{Fruhwirth:1987fm}, the algorithm has several aspects making it more suitable for FPGA implementation compared to global track fitting methods:

\begin{itemize}
\item {the matrices involved are small and are their size is independent of the number of measurements - }
\item {the only matrix inversion is of a small matrix.}
\end{itemize}


Figure~\ref{fig:KF} illustrates the \KF filtering and fitting process for a track candidate in the \rz plane of the barrel.
The \KF initially begins with an estimate of the track parameters and their uncertainties from the \HT array and $\eta$-$\phi$ segment assignment, known as the \emph{state}.



Stubs are iteratively added to the state and its covariance matrix, ordered 

Using the track parameters and their uncertainties from the \HT array and $\eta$-$\phi$ segment assignment as an initial estimate, or \emph{state}, the \KF iteratively adds stubs to the state in order of increasing radius.
With the addition of each stub, the state 
...
The line segments represent the predicted track trajectory at a given stage in the fitting process. The projected trajectory and its uncertainty is updated following the iterative addition of stubs in order of increasing radius. The two dashed line segments correspond to the projected trajectories of the track from two stubs that were compatible with the expected track trajectory at that stage. The state including stub 2b is discard
...
This section presents a number of simulation studies which were undertaken as part of the development of the \emph{TMTT} finder demonstrator system both before and following the 2016 review.
...
The development of a Kalman Filter was motivated by it being the optimal method of filtering a linear systems.
This made it ideally suited removing incorrectly assigned stubs from tracks and the removal of fake track candidates.
...
\subsubsection{Outcome of the $\chi^{2}$ Track Fit Studies}\label{subsubsec:chi2Conclusions}
