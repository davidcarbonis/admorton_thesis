...
For the simplest case of detecting straight lines, the Hough Transform describes a set of points $(x,y)$ in real space in terms of the gradient and intercept of a line in parameter space.
The point at which these lines intersect in the \emph{Hough}-space correspond to a line in real space. 

As such, the \HT is well suited to identifying tracks from a set of stubs.

This concept is illustrated in figure~\ref{fig:HT}
...
\begin{equation}
\frac r{2\,R} = \sin\left(\varphi-\phi\right) = \varphi-\phi\;
\label{eq:stub_R}
\end{equation}
