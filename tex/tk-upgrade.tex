...
For the general form  of a $\chi^{2}$ fit, a track composed of hits (\ie stubs) 

Initially the projection of the track, $f_{i}(overrightarrow{h})$, described as a function of its helix parameters, is linearly expanded around the estimate of the helix parameters $\overline{h}$:
...
Using Eqn.~\ref{eq:chi1}, the form of the track' $\chi^{2}$ can be expressed as:
...
By is minimising $\chi^{2}$, $\delta h$ can be :
...
By defining the matrices $D_{ij} = \frac{\partial f_{i}}{\partial h_{k}}$ and $M = D^{T} V^{-1} D$, Eqn.~\ref{eq:chi3} can be rewritten and solved for $\delta h$ as:
...
The track finding efficiencies, the mean numbers of reconstructed tracks per event in the entire tracker, and the number of those tracks which are fakes or duplicates, are given using the efficiency definitions described in Chapter~\ref{sec:TmttSimStudies}.
