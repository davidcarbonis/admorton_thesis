...
\editComment {Define track algo reco eff, perf and fakes here.}
...
Calculation of the derivatives for the barrel layer and endcap disk hits from this general form, including a correction factor for $\phi$ in the outer disks which do not point directly towards the interaction point, are given in~\ref{app:chi2}.

\subsubsection{Software Implementation}\label{subsubsec:chi2software}
The diminishing improvements from performing repeated iterations of Eqn.~\ref{eq:chi4}, as shown in Fig.~\ref{fig:}, can be improved upon by considering the residuals associated to each stub.
...
\editComment{Results done with 5000 ttbar+pu200 events. Larger stats runs will be done prior to final draft.}
...
The flexibility to reconstruct tracks down to a lower \pT threshold of 2\GeV may be desirable if the trigger requirements demand it and the impact of this potential requirement on the proposed Track Finder was studied 

\subsubsection{Hough Transform Optimisation}

The impact of this potential requirement on the proposed track-finder system was initially studied as one of the robustness studies for the December 2016 demonstrator review.
...
Both these initial layer dependent coefficients and the initial constant coefficient of a milliradian were subsequently fine tuned through \editComment{name of iterative method?} in order to recover as much tracking efficiency as possible.
...
During the comparative studies of these two approaches, it was observed in the results following the \DR stage that in contrast to the trend of the fraction of duplicates decreasing as \pT decreases, the fraction of duplications present increases above circa $3\GeV$, as shown in Fig~\ref{2GeVfracDups} (right).
The presence of this trend following in tracks produced by the \HT shown in Fig~\ref{2GeVfracDups} (left), prior to tracking fitting and \DR, implies that the\HT produces more duplicates at low \pT is the cause of this rather than a shortcoming of the \DR algorithm.
...
\includegraphics[width=0.47\textwidth]{CMS-bw-logo.pdf}
\caption{Plots showing tracking efficiency in low pT range with no changes (upper), and with HT cell merging and \KF optimisation(right).}
...
\item separate \KF $\chi^{2}$ cuts for the r-$\phi$ and r-z planes ....
 in order to reflect the lack of multiple scattering contributions to the r-z plane.
