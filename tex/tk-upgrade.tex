...
\caption{
Relative \pt resolution, $\phi_{0}$ resolution, $z_{0}$ resolution and $cot(\theta)$ resolution measured for primary reconstructed tracks in simulated \ttbar events with a $<\textrm{PU}>$ of 200 events for the floating point (red) and discretisedd mathematics (blue) implementations of the linearised $\chi^{2}$ fit algorithm for a single fitting iteration.
}
...
These studies included the development and evaluation of a linearised $\chi^{2}$ track fitting algorithm and determining the flexibility and robustness of the system if the minimum track reconstruction \pT threshold was reduced from 3\GeV to 2\GeV.

%% Discuss how good approx version was/number of fitting iterations
%% Discuss approx version comparible(?) performance in track reco/resolution wrt. kalman filter
%% Limitations wrt purity and endcap matrix element's tabulation
A linearised $\chi^{2}$ track fitting algorithm that used tabulated track derivatives and digitised variables and calculates was explored for the track fitting component of the TFP.

It was demonstrated that by performing multiple iterations of the algorithm was capable of fitting precise helix parameters to the track candidates found by the \HT, with those in the barrel being comparable to the expected offline resolution, and identifying and removing both hits that were incorrectly matched to genuine track candidates and unmatched track candidates.

Following the parallel development of both the linearised $\chi^{2}$ track fit and the \KF algorithms, it was decided that development of the former would be discontinued, as the \KF was capable of achieving both a higher track finding efficiency with 100.0\% purity for matched tracks and achieving $z_{0}$ and $cot(\theta)$ resolutions in the forward regions that were comparable to the expected offline resolution.
In addition, considerable progress had been made with a firmware implementation of the \KF.
...
%%%
Reducing the minimum track reconstruction \pT threshold to reconstruct 
The main issue ... the minimum track reconstruction \pT threshold   
%% Main issue = MS
