...
For the general form of a $\chi^{2}$ fit for a track, $f$, described by its helix parameters, $\overrightarrow{h}$, and the position of its hits (\ie stubs), $s_{i}$, the expected trajectory of the track, $f_{i}$, is initially linearly expanded around the estimate of the helix parameters $\overline{h}$:
...
These studies were initially undertaken as part of the the robustness studies required for the 2016 demonstrator review which focussed on recovering tracking efficiency below 3\GeV with the \HT.
Therefore, the results for these studies were produced using the flat barrel geometry.

Following the 2016 review, the initial studies into tracking with a pT threshold of 2\GeV were further developed by ... moidying ... to the \KF algorithm .
