...
Corrections are applied to the jet energies to account for the effects of \PU, non-uniform response of the detector and the residual differences 

The corrections 

\begin{itemize}
\item \textbf{L1 Pileup} - removes additional energy originating from \PU interactions from the jet energy for both data and MC simulated events.
\item \textbf{L2 Relative} - accounts for non-uniform detector response in $\eta$, derived using , and is applied for both data and MC simulated events.
\item \textbf{L3 Absolute} - corrects the \pT dependent scale and is derived 
\item \textbf{L2L3 Residual} - the \emph{L2 Relative} and \emph{L3 Aboslute} , residual jet energy scale differences between data and MC simulation.
\end{itemize}

A thorough description of the jet energy corrections, their determination and associated uncertainities are discussed in~\cite{Khachatryan:2016kdb}.
