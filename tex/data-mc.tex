...
The energy deposited by particles in the calorimeter is clustered in each sub-detector independently, except for the HF where each large cell can give rise to one cluster, to determine the energy and direction of the particles~\cite{CMS:2009nxa}.
When these clusters are combined with charged particle tracks,
...
\item \textbf{Topological clusters} - adjacent cells are summed together to form topological clusters.
\item \textbf{Particle Flow clusters} - 
\end{itemize}
...
As Kalman Filters assume solely Gaussian noise contributions and Bremsstrahlung is a non-Gaussian process, a Gaussian Sum Filter (GSF) algorithm~\cite{Adam:2003eca} is used to fit the track candidates.
