...
\editComment{Currently being rewritten/proofed/determined whether it'd be best suited to the bit where the MC samples are discussed}
%\paragraph{aMC@NLO}
...
\editComment{Worth discussing Hybrid and Multi-clustering algos for ECAL clustering? Wary of then having to expand on HCAL clustering... Better to just state where a more complete description is given.}
...
Electron track reconstruction is undertaken by the PF algorithm with two complimentary algorithms:
\begin{itemize}
\item \textbf{ECAL-based approach} - using the ECAL clusters
