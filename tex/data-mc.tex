...
The generation stage involves the use \emph{event generators} to simulate all aspects of a physics event from the initial protons to the hadronised remnants of the collision and the remnants of the initial colliding protons~\cite{Buckley:2011ms,Hoche:2014rga}.
...
This stage also includes the \emph{matching} of the separated particles produced by the ME stage to the lower \pT particles in the PS stage in order to provide a coherent description of the event.
Following the PS stage, the remaining particles undergo hadronisation using a non-perturbative modelling process using either the Lund String Model~\cite{Andersson:1983ia} or the Cluster Model~\cite{Winter:2003tt}.
