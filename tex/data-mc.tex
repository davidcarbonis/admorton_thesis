...
\begin{itemize}
\item \textbf{aMC@NLO} - aMC@NLO~\cite{Alwall:2014hca} is a package that considers tree-level and one-loop diagrams to produce matrix elements at NLO.
Considering the constructive and destructive interferences which result from considering the higher order and leading order cross section terms results in positively and negatively weightings respectively of the event.
...
POWHEG produces the hardest emission first through using the exact NLO ME and produces only positively weighted events when interfaced to a PS generator which order emissions by \pT.
The \pT ordering of emissions allows ...
\editComment{Documentation talks about \pT ordering or \pT veto - surely one implies the other? Need to check.}
