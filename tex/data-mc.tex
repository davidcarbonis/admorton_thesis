...
The first stage of the algorithm involves associating charged particle tracks with clusters in the ECAL and HCAL sub-detectors.
The linking of tracks to clusters is done by extrapolating a track's last measured hit in the tracker to the calorimeter systems.
If a track's expected position in the calorimeters are within the clusters' boundaries, these elements are linked together.
Similarly, Bremsstrahlung photons are


These linked elements are then used to sequentially reconstruct and identify different PF particles types.
As PF particles are identified, their associated tracks and clusters are removed from further consideration.

The different particle types are reconstructed in the following order: 
\begin{itemize}
...
\item charged hadrons are reconstructed from the HCAL clusters which are compatible with the remaining ECAL clusters and charged particle tracks. HOW
\item any remaining ECAL and HCAL clusters with no associated charged particle tracks are reconstructed as photons and neutral hadrons, respectively. HOW
\end{itemize}
...
If the extrapolated track lies within t
\end{itemize}
