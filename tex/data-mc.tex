...
The first part of this stage uses proton Parton Distribution Functions to assign 

is the modelling of the colliding protons and the hard scattering of their constituent partons where 


, which involves the use Parton Distribution Functions (PDFs) to assign fractions of the proton's momentum to the partons and perturbation theory to compute the Matrix Elements (ME) of the Feynman diagrams for the QCD and electroweak processes involved.

The second stage models result Parton Showers (PS) through an iterative process until the infrared cutoff scale for the shower is reached and perturbation theory is no longer applicable.
...
While there are several general-purpose event generators that can describe an event from the initial hadron collision to final state particles, such generators can be interfaced with 

they are typically used in conjunction with a specialist generator that that models a specific physics aspect (\ie ME calculations or PS simulation) or process (\eg tau decays) in order to provide a complete event.
...
\item \textbf{MADGRAPH5_aMC@NLO 2.2.2}~\cite{Alwall:2014hca} is the new version of MadGraph5 and the aMC@NLO generators which is capable of evaluating tree-level and one-loop Feynman diagrams to produce matrix elements at NLO.
...
As its name suggests, POWHEG produces the hardest emission first by using the exact NLO ME and generates only positively weighted events.
...
\item \textbf{PYTHIA 8:}~\cite{Sjostrand:2014zea} is a general-purpose generator that is capable of generating all aspects of a process to a complete event.
