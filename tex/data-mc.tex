...
The various event generators used in the production of MC samples used in this thesis are discussed in Chapter~\ref{subsec:eventGenerators}.
...
The DIGI stage uses the SIM output to produce the detector's electronics response, which then undergoes the same RECO process that data does, as described in Chapter~\ref{sec:reco}.
...
Therefore, the reweighting of the simulated samples is required and is discussed in Chapter~\ref{subsec:puSF}.
...
\item electrons and associated Bremsstrahlung photons and isolated photons are reconstructed, as described in Chapter~\ref{subsec:objReco-electrons}.
...
Muons tracks are independently reconstructed in both the inner tracker, as described in Chapter~\ref{subsec:tracksVertices}, and the muon chambers, where local reconstruction is undertaken using a Kalman Filter to build tracks from the innermost track segments of clustered DT and CSC hits outwards using DT, CSC and RPC hits.
...
This correction applies the jet energy corrections discussed in Chapter~\ref{subsubsec:JECs} to the PF jets with $\pT >15\GeV$ which are used in calculating the \MET.
