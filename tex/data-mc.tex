...
\editComment{Currently being rewritten/proofed/as originally event generators were to be discussed with simulated samples used in another chapter, but determined that it would be better placed here. Due to part written state, commented out for now as I'd rather get the bulk to you than keep you waiting.}
%\paragraph{aMC@NLO}
...
As it is essential that this radiated energy is collected in order to correctly determine an electron's initial energy, the ECAL clusters, known as \emph{superclusters} (SCs), have an extended window in $\phi$ as electrons' trajectories are bent in the $\phi$ plane by the magnetic field but photons' are not, causing an energy spread across $phi$.
...
As Kalman Filters assume solely Gaussian noise contributions and Bremsstrahlung is a non-Gaussian process, a Gaussian Sum Filter (GSF) algorithm~\cite{Adam:2003eca} is used in electron track fitting.
The computationally heavy nature of the GSF algorithm however, limits its use to refitting \KF track seeds which have a significant presence of Bremsstrahlung and for final track fitting~\cite{Khachatryan:2015hwa}.

Electron track reconstruction is undertaken by the PF algorithm, with seeds of the initial two or three hits in the tracker being constructed by two complimentary algorithms~\cite{•}:
\begin{itemize}
