...
The first stage of generation models the hard interactions between the partons of the two incoming protons.
Parton Distribution Functions are used to provide a detailed description of the internal structure of the two protons in their initial state and perturbative methods are used to determine the Matrix Elements (ME) of the hard process.
The second stage models the resultant Parton Showers (PS) through an iterative process until the factorisation scale for the shower is reached and perturbation theory is no longer applicable.

In order to combine the well separated particles described in the ME calculations with the lower \pT particles in the PS
