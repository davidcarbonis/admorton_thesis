...
The second stage models the resultant Parton Showers (PS) through an iterative process until the infrared cutoff scale for the shower is reached and perturbation theory is no longer applicable.
In order to combine the well separated particles described in the ME calculations with the lower \pT particles in the PS


The remaining particles in this stage undergo hadronisation using a non-perturbative modelling process using either the Lund String Model~\cite{Andersson:1983ia} and the Cluster Model~\cite{Winter:2003tt}.
