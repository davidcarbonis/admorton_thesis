...
The $U(1)_{Y}$ component of the theory has a single generator, and an associated gauge field $B_{\mu}$, which acts on and conserves weak hypercharge, $Y_{W}$.
...
The $SU(2)_{L}$ component of the theory has three generators, ${T_{i}} = \frac{{\sigma_{i}}}{2}$, which manifest as the gauge fields $W_{\mu}^{i}$ where $\bm{\sigma}$ are the pauli spin matrices and $i = 1, 2, 3$.
Under $SU(2)_{L}$ transformations, the left-handed fermion fields, $\psi_{L}$, transform as doublets and the right-handed fermion fields, $\psi_{R}$, as singlets.

As $SU(2)$ transformations are non-Abelian, the 

...

The four physically observed gauge bosons are related to the four gauge fields of the electroweak theory by:
...
For the charged weak interactions
...
Originally, in order to ensure that the SM Lagrangian remained invariant under weak isospin rotations, it required that the gauge bosons were massless.  
The reconciliation between the requirement for gauge invariance and the observation of massive weak gauge bosons was  achieved through the spontaneous symmetry breaking of the \emph{Higgs mechanism} proposed by Brout, Engler, Higgs,Guralnik, Hagen and Kibble~\cite{Englert:1964et,Higgs:1964pj,Guralnik:1964eu}.

The Higgs mechanism introduces a complex scalar field, known as the \emph{Higgs field}, that has a potential with a non-zero vacuum expectation value (VEV).
Infinite number of minima/ground states -> expanding field around arbitary choice of ground state break the EWS
While VEV itself  
Ellis:2013jnq

Following symmetry breaking, three of Higgs field's degrees of freedom mix with the weak gauge bosons, .
The remaining degree is freedom 
The Higgs mechanism also couples to fermions also 
...
As colour confinement prevents a hadron's constituent elements being determ, their internal structure is studied using an external probe.
