...
The Standard Model (SM) of particle physics describes all the known elementary matter particles and their interactions with the weak, strong, and electromagnetic forces.
The SM is formulated using QFT,  describe particles as excitations of quantum fields whose dynamics are typically described using the Lagrangian formalism~\cite{LagrangiansSM}.
...
Exotic hadrons formed of larger groupings of quarks can be also formed, with both tetraquark and pentaquark states having been observed by the LHCb detector~\cite{Aaij:2014jqa,Aaij:2015tga} and ....
...
% This right-aligns numbers in column, but centers them under column title.
 \begin{tabular}{llcllccc}
   ...
   & & down & \textit{$d$} & $4.8$ & $- \frac{1}{3}$ & $- \frac{1}{2}$ \\
   & \multirow{2}{*}{II} & charm & \textit{$c$}  & $2.3$ & $+ \frac{2}{3}$ & $+ \frac{1}{2}$ \\
   & & strange & \textit{$s$}  & $4.8$ & $- \frac{1}{3}$ & $- \frac{1}{2}$ \\
   & \multirow{2}{*}{II} & top & \textit{$t$}  & $2.3$ & $+ \frac{2}{3}$ & $+ \frac{1}{2}$ \\
   & & bottom &\textit{$b$}  & $4.8$ & $- \frac{1}{3}$ & $- \frac{1}{2}$ \\
   \hline
...
% This right-aligns numbers in column, but centers them under column title.
 \begin{tabular}{lccccc}
   ...
   \hline
   \multirow{2}{*}{Gluon }& $g$ & $0$ & $0$ & $r \overline{g}$, $r \overline{b}, g \overline{r}, g \overline{b}, b \overline{r}, b \overline{g} ,$ & $0$ \\
...
where ${\gamma}^{\mu}$ are the \emph{gamma matrices} (also known as the Dirac matrices), which are defined in ... and $\partial_{\mu}$ is the partial derivative operator.
...
If however, equation~(\ref{eq:diracLagrangian}) has a local $U(1)$ symmetry, then $\psi(x)$ transforms according to:
...
\begin{equation}
\mathcal{L}_{SM} = \mathcal{L}_{Gauge} + \mathcal{L}_{Fermion} + \mathcal{L}_{Higgs} + \mathcal{L}_Yukawa}
\end{equation}
...
The $\mathcal{L}_{Higgs}$ and $\mathcal{L}_Yukawa}$ terms arise as a consequence of the breaking of electroweak symmetry and describe the scalar spin-0 Higgs field and the interactions between the Higgs field and fermions and gauge bosons, respectively.

\subsection{Electroweak Theory}\label{subsec:QED}
\subsubsection{Quantum Electrodynamics}\label{subsec:QED}
Quantum Electrodynamics (QED) is the Abelian gauge theory that describes the interactions electromagnetic force between all electrically charged particles.
...
This property makes charged weak interactions unique in being the only interactions during which fermion flavour can change, violating parity (P)~\cite{Lee:1956qn,Wu:1957my} and charge-parity (CP) symmetries~\cite{Christenson:1964fg}.
...
where $\theta_{W}$ is the weak mixing or \emph{Weinberg} angle, which is defined in Equation~(\ref{eq:weakMixingAngle}) as a function of the coupling strengths of the weak hypercharge and weak isospin coupling constants, $g'$ and $g$ respectively.
...
As discussed in Section~\ref{subsec:weakForce}, the W$^{\pm}$ gauge bosons only interact with the left-handed components of the fermion field, $\psi_{L}$.
...
The Cabibbo-Kobayashi-Maskawa (CKM) matrix, in Equation~(\ref{eq:ckm}):
...
describes the proportion of the mass eigenstates $d$, $s$, and $b$ that are present in the weak flavour eigenstates $d'$, $s'$, and $b'$~\cite{Tanabashi:2018oca}.
As such, each of the elements of the CKM matrix describes the strength of the coupling, or decay fraction, for charged W boson decays.
...
Eventually it becomes more energetically favourable for a quark anti-quark pair to be produced.
...
As the top quark's mass is greater than the mass of a W boson it capable of decaying into an on-shell W boson and a down-type quark.

 (almost exclusive a bottom quark) and giving it a much short lifetime than the other quarks.
...
This results in the top quark being the only quark that decays before it can hadronise. 
...
Despite their relatively small cross sections, 

\ttV processes are important backgrounds to other processes.
Therefore, \ttV 
which require detailed understanding in order to be able to probe for new physics with similar or smaller cross sections, such as tHq and tZq production~\cite{Khachatryan:2014ewa}.
...
\caption{Leading order diagrams for single top quark production channels via weak processes: (a) s-channel, (b) t-channel and (c) tW production.}
...
\begin{itemize}
\item these processes form backgrounds for not only SM processes such as \ttbar, but for for Higgs and BSM physics searches, .
...
%\begin{table}[htbp]
%\topcaption {Standard Model cross sections for top .
%}
...
The first search for tZq was undertaken for the trilepton final state using data collected by the CMS detector at $\sqrt{s} = 8\TeV$.
