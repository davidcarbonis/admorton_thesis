...
This results in the \emph{colour confinement} of quarks, as when $\alpha_{s}$ is sufficiently large, it becomes more energetically favourable to create new $q\overline{q}$ pairs than to separate the partons further~\cite{ElectroweakStrong,Griffiths,devenish2004deep}.
Paer
When partons are liberated from hadrons, such as in the high energy hadron collisions of the LHC, the resultant shower of partons form new hadrons in a process known as hadronisation~\cite{Andersson:1983ia}.
	
In QED, higher order processes are increasingly less important to the calculation of the Matrix Element due to the electromagnetic coupling constant being considerably smaller than one.
In contrast however, higher order contributions in QCD become increasingly important as $\alpha_{s}$ increases, making higher order QCD calculations increasingly difficult to perform.
...
This allows for the short distance components to be described using perturbation theory, such as the hard scattering or hardons, while the long distance components are described using non-perturbative models, such as Parton Distribution Functions (PDFs).
PDFs
