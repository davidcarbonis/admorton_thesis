...
These concepts can be demonstrated by considering applying the $U(1)$ gauge symmetry of Quantum Electrodyanmics, the theory of electromagnetism, on the Lagrangian of a relativistic spin-$\frac{1}{2}$ free-fermion field (\ie electrons) with a wavefunction $\psi(x)$ and particles of mass $m$~\citep{1475-7516-2006-06-019}{QFT}:
...
As discussed in Section~\ref{subsec:weakForce}, the W$^{\pm}$ gauge bosons only interact with the left-handed components of a fermion field, $\psi$.
The left and right-handed components of $\psi$, $\psi_{L}$ and $\psi_{R}$, can be obtained by applying the projection operators  
...
The $\psi_{L}$ doublet consists either of a pair of left-handed fermions 
same generation up-type and down-type left-handed quark pair or a charged lepton and its associated neutrino:
...
The $\psi_{R}$ singlets consist of either an up-type or down-type quark or charged lepton


As the charged weak gauge bosons 
...
The Higgs mechanism introduces a complex scalar field, $\phi$, which has a potential, $V(\phi)$, with a non-zero vacuum expectation value (\textit{VEV}) and an infinite number of ground states, as shown in figure~\ref{fig:higgsPotential}.
...
Following the symmetry breaking, the interactions between the Higgs field and electroweak fields results in three of the Higgs fields degrees of freedom combining with 
The remaining degree of freedom manifests as a single massive scalar field

The Higgs field also allows for gauge invariant {Yukawa} terms can be included in the Lagrangian for the fermion fields
...
The probing 
Consequently, as a hadron consists of not only its constituent valence quarks but also a sea of gluons and quarks from the continuous gluon splitting inside it~\cite{coughlan2006ideas,evenish2004deep}.
The first evidence of the 
The probing of the proton through Deep Inelastic Scattering (DIS) 
