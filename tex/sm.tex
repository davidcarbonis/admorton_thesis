...
These new quarks form new hadrons with the separated quarks in a process known as hadronisation.
This \emph{colour confinement} is a direct consequence of asymptotic freedom and results in quarks only existing only in colour-singlet states~\cite{ElectroweakStrong,Griffiths,devenish2004deep}.

Given that $\alpha_{s}$ can rapidly change and be greater than one, QCD calculations cannot be solely determined solely through perturbation theory.
Consequently, for a given energy scale, QCD calculations are factorised into 



As perturbation theory cannot be used when $\alpha_{s}$ is not much smaller than one, QCD calculations are factorised  into their perturbative and non-perturbative components.
Therefore, 
