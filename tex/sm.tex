...
Consequently, quarks are said to be \emph{colour confined} and can only exist in colour-singlet states~\cite{ElectroweakStrong,Griffiths,devenish2004deep}.

Given that the constituent partons of a hadron cannot be directly observed as free particles,  
...
At sufficiently high energies, such at the hard interaction 
When $\alpha_{s}$ drops 
As such, for a given factorisation scale, QCD calculations are separated into their 

4fs/5fs

While $\alpha_{s}$ is small, perturbation theory can be used, hwowever, as at large energies.
