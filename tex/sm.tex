...
\begin{table}[htbp]
\topcaption {
The fundamental forces of nature and the SM gauge bosons which mediate them~\cite{Tanabashi:2018oca}.
}
\label{tab:bosons}
  \centering
  \resizebox{\textwidth}{!}{
...
A consequence of $\alpha_{s}$ increasing as quarks are increasingly separated is that quarks are confined to exist in a colourless state.
...
As $\alpha_{s}$ can rapidly change and be greater than one, it is extremely challenging to perform theoretical QCD calculations. 
While 
...
%In contrast to QED and its relatively small coupling constant, $\alpha_{s}$ can rapidly change and be greater than one, making 
%This not only increases the importance of higher order terms in QCD calculation and the difficulty and accuracy of the perturbative QCD calculations.
%At very high 
%
%partons to fragment/parton showering, which continues for as long as the partons have 

%link to PDF

% link to mu F and mu R
...
%While it may be initially assumed that \ttbar processes in association with vector bosons would be easier to measure, given the large production cross section for \ttbar processes when compared to single top production cross sections, this is only the case for \ttH.
%The tW coupling can only be probed through single top tW production given that for \ttW the W boson couples to the initial state quarks, as shown in Figure~\ref{fig:feyn_ttV}.
%While there is a tH coupling for both \ttH and tHq, only \ttH has been observed as the destructive interference between the tH and HW vertices suppresses tHq production~\cite{CMS:2018jsz,Maltoni:2001hu}.

%CMS has made measurements of \ttH, \ttW, and \ttZ production, all of which are consistent with the corresponding SM predictions~\cite{Sirunyan:2017uzs,Sirunyan:2018hoz}.
...
In contrast, \ttZ has a lower production cross section than the combined tZ and $\overline{\text{t}}$Z production cross sections~\cite{Campbell:2013yla}.
This is the result of the large number of particles present in the \ttZ final state suppressing \ttZ production.
...
tZq is a rare SM process where a single top quark is produced in association with a Z boson with an additional jet.
This process provides a unique precision probe of the top quark and the electroweak sector, as the Z boson is radiated off one of the quark legs, as shown in the top two rows of Feynman diagrams in figure~\ref{fig:feyn_tZq}, or from the exchanged W boson, as shown in the bottom left diagram in figure~\ref{fig:feyn_tZq}.
Therefore, in contrast to \ttZ, tZq not only sensitive to the top quark and Z boson coupling, but also to the WWZ coupling.
In addition, tZq is an important process to be understood as it forms irreducible backgrounds for other rare SM processes, such as tH, but also BSM processes such as Flavour Changing Neutral Currents~\cite{AguilarSaavedra:2004wm}
...
Both the expected result presented in this work and the CMS searches for tZq using the trilepton final state at $\sqrt{s} = 13\TeV$ use the reference next-to-leading order production cross section for tZq where the Z boson decays leptonically~\cite{Campbell:2013yla}:
...
%
% as the Z boson is radiated off one of the quark legs, as shown in Figures~\ref{fig:feyn_tZq}(a)-(e), or from the exchanged W boson, as shown in Figure~\ref{fig:feyn_tZq}(f).
%
%
