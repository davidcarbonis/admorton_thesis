...
If a symmetry does not possess a space-time dependence it is said to be \emph{global} and conversely, if the symmetry has a space-time dependence it is said to be \emph{local}.

As a QFT, the SM describes particles as excitations of fields whose dynamics are typically described using the Lagrangian formalism~\cite{LagrangiansSM}.
For the SM to describe the natural symmetries, and their associated conserved quantities, observed in nature, the SM Lagrangian must remain invariant to their transformations.

The impact of requiring the Lagrangian's invariance can be illustrated by considering the effect of applying the $U(1)$ transformations on the Lagrangian of a spin-$\frac{1}{2}$ fermion field,
...
where $\psi$ represents a relativistic spin-$\frac{1}{2}$ field, ${\gamma}^{\mu}$ are the Dirac matrices, and $m$ is the mass of the particle~\cite{QFT}.

If $U(1)$ is a global transformation, then $\psi$ transforms as 

\begin{equation}
\psi(x) \rightarrow \psi'(x) = e^{-i e \alpha}\psi(x)
\label{eq:globalTransformation}
\end{equation}

where $\alpha$ is an arbitrary phase, which leaves the Lagrangian in equation~\ref{eq:diracLagrangian} unchanged.
If however, $U(1)$ is a local transformation, then $\psi$ transforms as

\begin{equation}
\psi(x) \rightarrow \psi'(x) = \psi(x)e^{-i e \alpha (x) }
\label{eq:localTransformation}
\end{equation}

which results in the Lagrangian no longer being invariant, now transforming as  

\begin{equation}
\mathcal{L} \rightarrow \mathcal{L}' = \mathcal{L} + e \bar{psi}(x) \gamma^{\mu} \psi(x) \partial_{\mu}f(x)
\label{eq:localLagrangian}
\end{equation}

In order for 
A vector field, $A_{\mu}(x)$, associated with the $\psi(x)$ field can be introduced


Invariance can be restored by introducing a gauge field $A_{\mu}(x)$, associated with the $\psi(x)$ field, which transforms according to the gauge transformation. 

The minimal substitution in $\mathcal{L}$ which achieves this is the replacement of the derivative $\partial_{\mu}$  with the so-called ``covariant derivative''~\cite{QFT}
...
By applying the $SU(3)_{C} \times SU(2)_{L} \times U(1)_{Y}$ gauge symmetry to the SM Lagrangian, the gauge fields for the strong and electroweak forces, and their associated bosons, naturally emerge.
...
Despite the apparently disparate natures of the electromagnetic and weak forces, their unification into a single \emph{electroweak} interaction was independently developed by Abdus Salam and Steven Weinberg based 
...
The remarkable nature of asymptotic freedom makes performing theoretical calculations and experimental tests  of QCD extremely challenging.
In contrast to QED, where its relatively small coupling constant results in increasingly higher order processes having decreasing importance, higher order terms can become increasingly important in QCD as the strength of $\alpha_{s}$ can rapidly change and be greater than one.
As such, when QCD calculation
...
performing QCD calculations and 


Additionally 
When a low momentum probe, such as a photon, is used, only 

Understanding the nature of quarks and strong interactions is 
Our knowledge 
