...
Therefore, while $\alpha_{s}$ is sufficiently small inside a hadron for partons to behave as free particles, increasingly large amounts of energy are required to pull the hadron apart.
If a pair of quarks are separated 
When 

Consequently, when it becomes more favourable to create 

Consequently, quarks are \emph{confined} to exist in colour-singlet states~\cite{ElectroweakStrong,Griffiths,devenish2004deep}.

The liberation of partons from their constituent hadrons is a common occurrence in the high energy hadron collisions, such as those at the LHC, 

These new quarks and the separated quarks form new hadrons in a process known as hadronisation.


In QED, higher order processes contribute less to the Matrix Element due to the alpha EM -ve
In contrast however, when higher order contributions in QCD become increasingly important as alphaS increases. 
...
%Given that the strong coupling constant can rapidly change and be greater than one, unlike QED, QCD calculations cannot be solely determined solely through perturbation theory.
%
%Consequently, for a given energy scale, QCD calculations are factorised into components where 
