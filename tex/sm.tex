...
If a symmetry has no space-time dependence it is said to be \emph{global} and conversely, if the symmetry does have a space-time dependence it is said to be \emph{local}.

These concepts can be demonstrated by considering applying the $U(1)$ local symmetry of Quantum Electrodyanmics (QED), the theory of electromagnetism, on the Lagrangian of a relativistic spin-$\frac{1}{2}$ free-fermion field, for a wavefunction $\psi(x)$ and particles of mass $m$~\ref{eq:diracLagrangian,QFT}:
...
This field can be simply introduced by replacing the derivative $\partial_{\mu}$ with the \emph{gauge covariant derivative}~\cite{QFT) $D_{\mu} = \partial_{\mu} - i A_{\mu}(x)$.
