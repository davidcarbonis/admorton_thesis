...
For the combination of both final states, at 95\% CL, the expected signal strength for tZq production was determined to be $X^{+Y}_{-Z}$, corresponding to an expected significance of $X_{-Z}^{+Y} \sigma$.
Using the reference NLO cross section of $\sigma (tZq, Z \rightarrow l^{+} l^{-}$) = 94.2 fb, given in Section~\ref{}, this results corresponds to an expected cross section of $X^{+Y}_{-Z}$

This measurement is within the uncertainties of the SM prediction of $94.2^{+1.9}_{-1.8} \pm 2.5$ fb~\cite{Campbell:2013yla} and the cross section measurements of $70.65 \pm 25.9$ and $137^{+45}_{-40}$ made by the ATLAS and CMS searches using the trilepton final state at $\sqrt{s} = 13 \TeV$~\cite{Aaboud:2017ylb,Sirunyan:2017nbr}.
...
While the searches for both the dilepton and trilepton final states are currently statistically limited, the amount by which they impacted by statistics is a consequence of size of their associated backgrounds.
