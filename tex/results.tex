...
All of the nuisance parameters are described by a probability density function (pdf)~\footnote{Not to be confused with Parton Distribution Functions (PDFs)}.
...
Therefore, the likelihood for the entire dataset is the product of the Poisson probabilities, $\mathcal{P}$, for all bins and the nuisance parameters' probability density function, as given by equation~\ref{eq:poissonLikelihood}.

\begin{equation}
\mathcal{L} ( n_{i} | \mu , \theta ) = 
\prod\limits_{i=1}^{\N} _{i} \mathcal{P} \big( n_{i} | \mu s_{i}(\theta) + b_{i}(\theta) \big) \;
\label{eq:poissonLikelihood}
\end{equation}



In order to determine the signal strength 
