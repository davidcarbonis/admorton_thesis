...
For the combination of both final states, at 95\% CL, the upper limit on the cross section for tZq production is expected to be $X^{+Y}_{-Z}$ which corresponds to an expected significance of $X_{-Z}^{+Y} \sigma$.
This limit is within the uncertainties of the SM prediction of $0.0758^{Y}_{Z}$ pb~\cite{} and the upper limits of $X$ and $Y$ set on the cross section by the ATLAS and CMS searches using the trilepton final state at $\sqrt{s} = 13 \TeV$~\cite{Aaboud:2017ylb,Sirunyan:2017nbr}.
...
In contrast to the trilepton final state, the dilepton final state 
