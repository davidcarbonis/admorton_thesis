...
Using the reference NLO cross section of $\sigma (tZq, Z \rightarrow l^{+} l^{-}$) = 94.2 fb~\cite{Campbell:2013yla}, this results corresponds to an expected upper limit on the cross section of 189.5 fb. %$94.2^{+95.3}_{-98.4475}$.
This is both consistent with the SM prediction and the cross sections of measurements of $0.75 \pm 0.28$ and $1.45^{0.48}_{-0.42}$ that were made of the signal strength by the ATLAS and CMS collaborations, respectively, using the trilepton final state at $\sqrt{s} = 13 \TeV$~\cite{Aaboud:2017ylb,Sirunyan:2017nbr}.
...
The observed significances of these measurements made using the trilepton final state at $\sqrt{s} = 13 \TeV$ by the ATLAS and CMS collaborations were 4.2 $\sigma$ and 3.7 $\sigma$, respectively.
...
Therefore, as the lack of sufficient statistics is the main factor limiting the significance of the expected result presented, it is essential that this search is extended to include additional data collected by the CMS experiment if tZq is to be observed using the dilepton final state.