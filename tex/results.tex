...
The expected result of $94.2^{+95.3}_{-98.4475}$ for the tZq production cross section made using the dilepton final state is consistent with ATLAS and CMS collaboration measurements at $\sqrt{s} = 13 \TeV$.
While searches for both final states are currently statistically limited, the expected significance of $0.95 \sigma$ using the dilepton final state is considerably smaller than the significances of 4.2 $\sigma$ and 3.7 $\sigma$, respectively, reported by the ATLAS and CMS collaborations using the trilepton final state.
The large differences between the expected significance for the dilepton final state measurement and the observed significances for the trilepton final state measurements is a consequence of the 
size of their associated backgrounds.
