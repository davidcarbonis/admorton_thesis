...
As the PDFs are experimentally derived from various experiments, which requires the propagation of the uncertainties associated with each experiment, 

the impact of the systematic uncertainties associated with the PDF measures are require the analysis evaluated.

The NNPDF3.0 family of PDF sets is considered for all of the samples considered, with the LO and NLO and four and five flavour scheme sets used depending on whether the sample was produced at LO or NLO or using the four or five flavour schemes.
For each PDF set, one hundred alternative event weights are stored as per-event weights in the LHE event header for almost all of the MC samples considered.
An estimation of the of the PDF uncertainties are derived from these alternative event as the root mean square of the difference between the nominal and alternative event weights

The exception to his are the single top tW-channel samples as it was not possible at the time the samples were produced  to generate per-event weights to account for both the matrix element factorisation and renormalisation scales and PDF variations for this particular process.
In order to evaluate the impact of the uncertainties associated with the PDF measurements for the tW-channel samples, the LHAPDF (Les Houches Accord Parton Distribution Function) library is used to access both the nominal PDF weight and 
50 eigenvalues from the NNPDF3.0 set to provide one hundred alternative event weights to be considered.
Like the PDF variations stored in the LHE event header, the root mean square of the difference between the nominal and alternative event weights is used determine the appropriate systematic uncertainty.
...
CMS uses the pixel detector, DTs, HF, the Fast Beam Conditions Monitor and Pixel Luminosity Telescope to monitor and measure the instantaneous and integrated luminosity and Van der Meer (VdM) scans during dedicated LHC runs to calibrate the absolute luminosity scale calibrations of the detectors.

The Pixel Cluster Counting (PCC) method, using the pixel detector, has been used throughout Run 2 of the LHC by the CMS Luminosity Group to measure the luminosity delivered and the uncertainty associated with it.
The PCC method was chosen as 
