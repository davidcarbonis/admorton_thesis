...
Rather than tuning the hyperparmeters for optimal classification performance by hand or a computationally expensive and time consuming grid search, a function describing classifier performance as a function of the set of hyperparameters, $f(\theta)$, was considered.

the minima of a function describing classifier performance as a function of the set of hyperparameters, $f(\theta)$, is determined.

As such a function is a black box, the minima cannot be directly calculated, but they can however, but can be approximated through regression. 

The \emph{Scikit-Optimize} library was used to evaluate $f(\theta)$ using a regression model 

Minima found from this 
