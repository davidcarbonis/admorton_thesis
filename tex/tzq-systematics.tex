...
The final event yield after applying the full selection criteria and simulation corrections for the signal region on the simulated samples


 including non-prompt lepton contribution estimated from data (see Section~\ref{sec:NPLs}), compared to data is shown in table~\ref{tab:signalYields}.
...
The final event yields after applying for all of the selection criteria and simulation corrections for the zero b-tagged jets control region is given in table~\ref{tab:zPlusControlYieldsOld} and the \MET and inverted W boson mass cut control region in table~\ref{tab:zPlusControlYieldsNew}.
...
The final event yields following the application of the \ttbar control region selection criteria are shown in table~\ref{tab:ttbarCR}.
The resultant control region  to produce an enriched region that has a purity of 97.5\%, with 70\% of the remaining backgrounds being from tW single top production.
...
Table~\ref{tab:hyperparameters} lists the set of hyperparameters and their values for the $ee$ and $\multicolumn{•}{•}{•}\mu$ channels whose optimal values differ from their default values.
