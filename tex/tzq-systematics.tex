...
The final event yields for the signal region, including non-prompt lepton contributions(see Section~\ref{sec:NPLs}), for data and simulation are shown in table~\ref{tab:signalYields}.
...
\hline
$N_{data}^{OS non-prompt}$: & 69.47$\pm$0.76 & 114.01$\pm$1.25 \\
\hline
\end{tabular}
...
From these yields, it was observed that 94\% of the simulated events in the 0-bjet region were produced by Z+jets processes compared to 82\% in the $m_{W}^{inv}$ region. 
...
Z+jets (NLO): & 6922.4$\pm$107.9 & 13147.4$\pm$151.8 & 20069.8$\pm$259.7    \\
\hline
Total MC (LO): & 2773.3$\pm$69.7 & 5394.7$\pm$99.9 & 8168.0$\pm$169.6    \\
Total MC (NLO): & 7436.2$\pm$123.1 & 14178.4$\pm$174.5 & 21614.6$\pm$297.6    \\
\hline
Data: & 2940.0$\pm$64.2 & 5718.0$\pm$87.2 & 8658.0$\pm$151.4    \\
\hline
\end{tabular}
\end{table}

Based on the yields in both tables~\ref{tab:zPlusControlYieldsOld}-\ref{tab:zPlusControlYieldsNew}, it was also observed that the NLO Z+jets sample does not correctly describe the normalisation of the Z+jets process.

Further studies into the NLO sample were done following the application of a simple scale factor to normalise the NLO Z+jets sample.

During these studies, it was observed that unlike the LO sample, the NLO sample poorly described the jet multit
 before the jet selection criteria and jet corrections are applied, that the 
As illustrated in Figure~\ref{fig:zPlusCR_nJets}, 
It was also observed that following the application of the jet selection criteria and jet corrections that the NLO sample described the transverse momenta of the subleading jets better than the LO sample, as shown in Figure~\ref{fig:zPlusCR_jetPt}.
...
While the NLO sample does describe the transverse momenta of 
 
It was decided to use the LO sample to model the Z+jets background.
...
\includegraphics[width=0.47\textwidth]{figs/background-estimation/plots/unblinded/DY_control_old_prompt_mumu_DYamcatnlo/nJets_lepSel.pdf}
\caption{
The distributions of the number of jets in the zero b-tagged jets Z+jets control region following the lepton selection criteria and associated simulation corrections for the LO (left) and NLO (right) Z+jets samples in the $\mu\mu$ channel.
}
...
\includegraphics[width=0.47\textwidth]{figs/background-estimation/plots/unblinded/DY_control_old_prompt_ee_DYamcatnlo/leadingJetPt_jetSel.pdf}
\\
...
\includegraphics[width=0.47\textwidth]{figs/background-estimation/plots/unblinded/DY_control_old_prompt_ee_DYamcatnlo/secondJetPt_jetSel.pdf}
\\
...
\includegraphics[width=0.47\textwidth]{figs/background-estimation/plots/unblinded/DY_control_old_prompt_ee_DYamcatnlo/thirdJetPt_jetSel.pdf}
\\
...
\includegraphics[width=0.47\textwidth]{figs/background-estimation/plots/unblinded/DY_control_old_prompt_ee_DYamcatnlo/fourthJetPt_jetSel.pdf}
\caption{
The distributions of the leading four jets \pt in the nominal Z+jets control region following the application of the jet selection criteria and simulation corrections for the LO (left) and NLO (right) Z+jets samples in the $ee$ channel.
}
...
The final event yields for the \ttbar control region are given in table~\ref{tab:ttbarCR}.
The selection criteria produce an enriched region where 97.5\% of the expected events are produced by \ttbar production.
Of the remaining processes, 70\% are expected to be from tW single top production.
...
Examples of this observed good agreement for the final event selection are illustrated by the distributions of the number of jets and b-jets in Figure~\ref{fig:ttbarCR_nJets}; the \pT of the four leading jets in Figure~\ref{fig:ttbarCR_jetPt}; and the invariant mass and \pT of the two leptons in Figure~\ref{fig:ttbarCR_leptons}

Given the good agreement observed between data and simulation for both the overall normalisation and the distributions of individual variables, it was determined that the \ttbar MC sample accurately modelled data.
...
The uncertainty associated with the matching threshold used by POWHEG was evaluated using dedicated samples.


 where the matching threshold energy is varied up and down by one standard deviation~\cite{CMS:2016kle}.
This source of uncertainty is evaluated for the \ttbar and single top t-channel backgrounds generated with POWHEG using dedicated simulation samples, which are listed in table~\ref{tab:theorySampleList} as ``matching''.
...
The single largest theoretical source of uncertainty is 
...
\begin{tabular}{cccc}
   ...
   \hline
    ...
    zl1Quark2DelR & $\Delta R$ between the leading lepton and W boson's subleading jet & $4.093\times 10^{-2}$ & - \\
    thirdJetPt & Third jet \pt & $3.720 \times 10^{-2}$  & $3.753 \times 10^{-2}$  \\ 
    zlb1DelR & $\Delta R$ between the Z boson and leading b-tagged jet & - & $3.245 \times 10^{-2}$ \\    
    zEta & Reconstructed Z boson $\eta$ & - & $2.111 \times 10^{-2}$ \\
    zlb2DelR & $\Delta R$ between the Z boson and subleading b-tagged jet & $2.627\times 10^{-2}$ & - \\
