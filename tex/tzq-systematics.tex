...
The single top tW-channel samples are the exception to this as at the time of their generation 
it was not possible to generate per-event weights to account for both the matrix element factorisation and renormalisation scales and PDF variations for this process.
Therefore, in order to estimate the PDF uncertainties for the tW-channel samples, the LHAPDF (Les Houches Accord Parton Distribution Function) library is used to access both the nominal PDF weight and 50 eigenvalues from the NNPDF3.0 set to provide one hundred alternative event weights to be evaluated.
...
Throughout Run 2 of the LHC, the CMS Luminosity Group has measured the luminosity delivered with the pixel detector using the Pixel Cluster Counting (PCC) method.
 and Van der Meer (VdM) scans during dedicated LHC runs to calibrate the absolute luminosity scale calibrations of the detectors.
