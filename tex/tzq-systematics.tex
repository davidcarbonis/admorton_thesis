...
The efficiencies calculated for data, MC simulation and the resultant corrective scale factors applied to simulation are given in table~\ref{tab:triggerSFs} with their associated statistical uncertainties.
...
% This right-aligns numbers in column, but centers them under column title.
 \begin{tabular}{lccc}
   ...
   $\mu\mu$ & 1.0 $\pm$ & 0.99192 $\pm$ 0.00074 & 0 $\pm$ 0  \\
...
These studies included the comparison of the trigger efficiencies in simulation for \ttbar and Z+jets, the independence of the \MET trigger selection and the impact of applying the later stages event selection in the analysis performed. 

\editComment{Add bit about \ttbar and Z+jets following revised trigger SFs}
...
Table~\ref{tab:triggerCorrelation} shows that the value of found $\alpha$ for each channel only exhibits small differences from unity.
...
\editComment{Add table with trigger efficiencies post jet and b-jet cuts following revised trigger SFs}

Given the statistical uncertainties involved and the results of the studies briefly described above, it was determined that a systematic uncertainty of 1.5\%/1.5\%/2\% for the $ee$, $\mu\mu$, and $e \mu$ channels respectively would be sufficient.
