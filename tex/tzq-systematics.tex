...
Therefore by extending boosting (or bagging) to decision trees, the resultant forest of \emph{Boosted Decision Trees} produces much more effective classifier than a single tree which is also more resilient to fluctuations in the training sample.
...
Adaboost adjusts the weighting assigned to both misclassified objects and the weak learner following each iteration so that the best performing classifiers are trained to classify the most difficult to classify objects.
In contrast gradient 

boosting identifies hard to classify objects by their large residuals  
 to fit the weak learners not to the reweighted events but to 
