...
As seen in table~\ref{tab:zPlusControlYieldsOld}, while the Madgraph (LO) sample describes the overall normalisation well, it is clear that there is at the very least a normalisation issue with the aMC@NLO (NLO) Z+jets sample.
...
\caption{
The distributions of the number of jets in the zero b-tagged jets Z+jets control region following the full selection criteria, except the number of jets, and associated simulation corrections for Madgraph (left) and aMC@NLO (right) MC samples in the $\mu\mu$ channel.
}
...
The uncertainty associated with the choice of the energy threshold at which the ME calculations are matched to the PS is evaluated using dedicated matching samples where the matching threshold energy is varied up and down by one standard deviation~\cite{CMS:2016kle}.
...
The features chosen by this process for the $ee$ and $\mu\mu$ channels are given in table~\ref{tab:selectedBdtVariables} and 
...
Figure~\ref{fig:inputFeaturesDataSimAgreement} shows the good agreement observed between simulation and data for the features selected, demonstrating their reliability.
