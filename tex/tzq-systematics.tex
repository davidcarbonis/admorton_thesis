...
The impact of applying for all of the selection criteria and simulation corrections for the zero b-tagged jets control region is given in table~\ref{tab:zPlusControlYieldsOld} and the \MET and inverted W boson mass cut control region in table~\ref{tab:zPlusControlYieldsNew}.
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.47\textwidth]{figs/background-estimation/plots/unblinded/prompt_mumu_ttbarInc/nJets_lepSel.pdf}
\includegraphics[width=0.47\textwidth]{figs/background-estimation/plots/unblinded/prompt_mumu_DYamcatnlo_noSF/nJets_lepSel.pdf}
\caption{
...
In order to consider the impact of the uncertainty associated with the choice of factorisation ($\mu_{F}$) and renormalisation scales ($\mu_{R}$) used in the generation of the simulation samples, they are simultaneously varied up and down by factors of 2 and 0.5, respectively.
...
The uncertainty associated with this choice of matching threshold was evaluated using dedicated samples where the matching threshold energy is varied up and down by one standard deviation~\cite{CMS:2016kle}.
...
At each node in the decision tree, a single input variable (\emph{feature}) is compared against a threshold to determine which of the next two nodes it'll be sent to.
...
The methods used for determining the optimal features and model hyperparameters for this BDT are described in the following subsection.
...
\subsubsection*{BDT Feature Optimisation}
...
As training a BDT with all of the possible features increases its vulnerability to overtraining, as correlated features are considered multiple times and statistical fluctuations in poorly discriminating features may give the illusion of discriminative power.
...
As each feature is removed from consideration the probability a randomly chosen signal event has a higher discriminator value than a random background event is recorded.
From this list, the highest ranked features are selected for use in the BDT.
...
\caption{
The correlations between the features chosen for use with the BDT for the signal (top) and background (bottom) samples for the $ee$ channel.}
\label{fig:corr_ee}
\end{figure}
...
\caption{
The correlations between the features chosen for use with the BDT for the signal (top) and background (bottom) samples for the $\mu\mu$ channel.}
\label{fig:corr_mumu}
\end{figure}

Figure~\ref{fig:inputFeaturesDataSimAgreement} shows that good agreement was observed between simulation and data for the selected features, demonstrating that they are reliable features.

Given the number of features used, when a feature is used by both the $ee$ and $\mu\mu$ channels, only the distributions for the one of the channels are given in this chapter.
...
\caption{
Distributions comparing the agreement between data and simulation for the variables used as features in the BDT training.}
\label{fig:inputFeaturesDataSimAgreement}
\end{figure}

%% HYPERPARAMETERS
\subsubsection*{BDT Hyperparmeter Optimisation}
Instead of tuning the choice hyperparmeters for optimal classification performance either by hand or a time and computationally expensive grid search, a regression model was constructed and evaluated using the \emph{Scikit-Optimize} library~\cite{scikit-optimise}.
This regression model was constructed using a Gaussian process which identifies of optimal hyperparmeters as the minima of a mteric used to rank the hyperparmeters considered.
 
The hyperparameters considered were:
\begin{itemize}
\item \textbf{gamma:} minimum loss reduction required to make a further partition on a node.
\item \textbf{learning\_rate:} a scale factor/weighting of the weights of the new features that have been trained on. The smaller the value this is, the slower the BDT learns but is less likely to overtrain.
\item \textbf{max\_depth:} the maximum depth of the trees grown.
\item \textbf{min\_child\_weight:} minimum number of instances required for a node to be partitioned.
\item \textbf{n\_estimator:} number of trees grown.
\item \textbf{reg\_alpha:} a regularisation tern for the tree weights.
\item \textbf{reg\_lambda:} a regularisation tern for the tree weights.
\item \textbf{subsample:} the proportion of the training sample which is randomly chosen for training a tree for each iteration. This is used to prevent overtraining.
\end{itemize}
...
As the response of the BDT classifier for both the training and testing samples are consistent with each other, this implies that little to no overtraining has occurred. 
...
Figures~\ref{fig:inputFeaturesDistributions_ee} and~\ref{fig:inputFeaturesDistributions_mumu} shows the distributions of the selected features in the signal and background samples for the $ee$ and $\mu\mu$ channels, respectively.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.97\textwidth]{figs/background-estimation/plots/vars_ee.pdf}
\caption{
Distributions of the features chosen for use with the BDT for the signal and background samples for the $ee$ channel.}
