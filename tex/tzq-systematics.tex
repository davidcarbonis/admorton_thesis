...
\editComment{The following below is to be updated ASAP. Muon POG has found that the muon tracking SFs are no longer necessary - were the result of buggy derivation. Initial tests reveal that \pT and $\eta$ binned SFs for $\mu\mu$ may not be required...}
...
As it was observed that while the event yields in the signal region were correctly described when the Madgraph sample was used, with there aMC@NLO sample there was a dramatic normalisation offset.
...
A decision tree in its simplest form is a series of sequential binary decisions (nodes) on a single input variable or \emph{feature} at a time in order classify an event as signal or background.
Given that the decision criteria for each node is dependent on the decisions of the preceding notes, decision trees have the potential to obtain better separation between signal and background processes through individual cuts on isolated variables.

Despite such an single isolated tree being unlikely to be an efficient classifier, this tree still contains some knowledge about the underlying structure of the classification problem.
\emph{Boosting} aims to take advantage of this knowledge by iteratively training trees but following each iteration the data trained on is reweighted so .


create a strong learner from this weak learner by iteratively training 

improve the performance of this weak learner by iteratively training trees, 
This process, known as \emph{boosting}, 



\emph{Boosting} extends the concept of a decision tree from a single tree to a forest of trees with the aim of both stabilising the response and enhancing the performance of the decision trees.
This process involves training multiple trees in succession on the same training sample

.
At the end of this process all the trees are combined into a single classifier given by the weighted average of the trees, thus creating a strong learner from an ensemble of weak learners
This classifier produces a discriminant value used to classify an event, which typically ranges from -1 to +1 in order to denote how background or signal-like the event is.
...
The \emph{Gradient Boost} algorithm~\cite{Friedman:GradientBoosting} aims to improve its classification performance by identifying 
...
Following a series of studies into evaluating of a number of Boosting and Bagging algorithms for decision trees, which simulated background processes should be included in the training processes, and the use of multiple BDTs for different backgrounds, it was determined that the \emph{eXtreme Gradient Boosting} (XGBoost) implementation of the Gradiant Boost alogorithm for a single BDT trained on all the MC simulation samples gave the optimal performance for this search~\cite{xgboost}.
...
For each of the 150 iterations considered, $f(\theta)$ is evaluated across four subsets of 80\% of the training data using the the k-fold cross-validation scheme.
...
Out of these averaged final metrics, the classifier with the best $(\theta)$ was retrained on all the training data and used to produce the optimal BDT.
