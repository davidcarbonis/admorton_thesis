...
Due to the prescaling of all of the electron triggers considered during the start of the most luminous runs (as discussed in Section~\ref{sec:triggerStrategy}) the $ee$ channel uses a reduced dataset corresponding to 35.8\fbinv where none of the triggers considered were prescaled.
...
The triggers efficiencies for the analysis were determined using the \emph{cross-trigger} methodology used in~\cite{Khachatryan:2016kzg}.
...
between simulation and data for the event yields at each stage of applying selection criteria and simulation corrections, as shown in Figure~\ref{fig:cutFlow}, and also
...
Good agreement is also observed for the distributions of the reconstructed variables.
...
Based on the yields in both tables~\ref{tab:zPlusControlYieldsOld}-\ref{tab:zPlusControlYieldsNew}, it was also observed that the NLO Z+jets sample did not correctly describe the normalisation of the Z+jets process.
...
While most of the experimental uncertainties have a smaller impact on the event yields compared to the theoretical uncertainties, the JER and PDF uncertainties are observed to have a greater impact on the Z+jets and tZq processes, respectively, than any of the theoretical uncertainties.
