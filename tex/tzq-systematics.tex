...
Due to the prescaling of both ... electron triggers during the start of the most luminous runs, as discussed in Section~\ref{sec:triggerStrategy}, the $ee$ channel uses a reduced dataset of 35.6\fbinv where none of the triggers considered were prescaled.
...
Such scale factors are usually derived as a function of \pt and $\eta$ so as to fully account for the variation of the detector response in both.
These scale factors are used to correct the simulation for any discrepencies in the lepton identification, isolation and reconstruction efficiencies, b-tagging efficiencies, the poor modelling of pileup in simulation, and the detector resolutions observed in data.
...
As described in Section~\ref{subsubsec:JECs}, the Jet Energy Corrections are applied to account for the non-uniform response in \pT and $\eta$ of the detector by comparing the differences between the generator level and detector level responses and any residual differences between simulation and data.
