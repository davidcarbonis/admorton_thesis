...
As illustrated in Figure~\ref{fig:decisionTree}, a decision tree is a series of sequential binary decisions (nodes).
Each node 
 are made on a single input variable (\emph{feature}), $x_{i}$, until a final decision is made at a \emph{leaf node} and the object is classified as either signal (S) or background (B).
...
One of the potential issue with 
...
While bagging does not improve a weak classifier in the 
...
The alternative weights used by both methods represent
...
The factorisation ($\mu_{F}$) and renormalisation scales ($\mu_{R}$) used at the ME and PS levels are parametrised as functions of $Q^{2}$. 
