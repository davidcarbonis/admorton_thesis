...
Using a dataset corresponding to 35.86\fbinv, the expected upper limit on the production cross section has been determined to be $X_{Y}^{Z}$ fb.
This corresponds to an EXPECTED signal significance of $999 \sigma$ when compared to the background only hypothesis, which is consistent with the Standard Model prediction and the limits set by searches for the final state where the Z boson and W boson decay leptonically.
