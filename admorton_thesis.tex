...
\usepackage[table-align-uncertainty=true,separate-uncertainty=true]{siunitx}
...
\newcommand{\ttZ}{$\text{t}\overline{\text{t}}\textrm{Z}$\xspace}
\newcommand{\ttbarZ}{$\text{t}\overline{\text{t}}\textrm{Z}$\xspace}
\newcommand{\ttW}{$\text{t}\overline{\text{t}}\textrm{W}$}
\newcommand{\ttbarW}{$\text{t}\overline{\text{t}}\textrm{W}$}
\newcommand{\ttH}{$\text{t}\overline{\text{t}}\textrm{H}$\xspace}
\newcommand{\ttbarH}{$\text{t}\overline{\text{t}}\textrm{H}$\xspace}
...
\author{Alexander David John Morton}
...
Using a dataset corresponding to 35.9\fbinv, signal strengths of $6.21_{-2.67}^{+2.34}$ and $4.73_{-2.02}^{+1.92}$ were measured for this process when the Z boson decays into a pair of electron or muons, respectively, and the W boson decay hadronically.
These measurements correspond to an observed (expected) signal significance of $2.72\sigma$ ($0.46\sigma$) and $2.50 \sigma$ ($0.54\sigma$), respectively, when compared to the background-only hypothesis.
...
\input{tex/appendix_triggerEfficiency.tex}
